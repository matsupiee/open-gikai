...
- 新テーブル `topics` / `meeting_topics` を追加
  - `topics`: `id`, `municipality_code`, `canonical_name`, `aliases TEXT[]`, `description`
  - `meeting_topics`: `meeting_id`, `topic_id`, `relevance`, `digest`
...
`ask.ts` にデフォルトで保存する実装を入れ済み。`apps/meeting-summarizer/runs/YYYYMMDD-HHMMSS.md` に以下を書き出す:

- ユーザー質問 / preset / model / 自治体
- 各イテレーションのツール呼び出し（args, result）を JSON ブロック
- 最終応答または打ち切り理由 / エラーメッセージ
- 入出力トークン・API 呼び出し回数・推定コスト（Flash / Flash-Lite の料金テーブル 2026-04）

一時的な実行でログを残したくないときは `--no-save`。`runs/` は gitignore 済み。
...
`apps/meeting-summarizer/src/agent-presets.ts` に 4 プリセットを実装し、`--preset` で切り替え:

- `default`: 時系列で議論の流れを整理（従来の形式）
- `member`: 議員視点（ある議員が追い続けている議題）
- `policy`: 政策視点（同一政策のフェーズ別変遷）
- `compare`: 比較視点（複数議題の関連 — `find_meetings_with_topics` を積極的に使う）
