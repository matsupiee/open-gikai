...
- 文字コード: UTF-8
- 特記: DiscussVision Smart を採用。議会映像配信、議員発言一覧、キーワード検索機能あり。会議録テキスト（議事録）は公開されていない。発言内容のサマリー（`content` フィールド）と発言者情報のみ API から取得可能。全期間にわたって `minute_text` が空、`vtt_name` が null。
...
| テナント設定ファイル | `https://smart.discussvision.net/smart/tenant/nozawaonsen/WebView/js/tenant.js` |
...
| ページ | URL パターン |
| --- | --- |
| 会議名一覧（映像） | `https://smart.discussvision.net/smart/tenant/nozawaonsen/WebView/rd/council_1.html` |
| 会議名一覧（会議録） | `https://smart.discussvision.net/smart/tenant/nozawaonsen/WebView/rd/council_2.html` |
| 日程・発言一覧 | `https://smart.discussvision.net/smart/tenant/nozawaonsen/WebView/rd/schedule.html?council_id={council_id}&schedule_id={schedule_id}` |
| 発言詳細（動画） | `https://smart.discussvision.net/smart/tenant/nozawaonsen/WebView/rd/speech.html?council_id={council_id}&schedule_id={schedule_id}&playlist_id={playlist_id}&speaker_id={speaker_id}` |
...
```
GET https://smart.discussvision.net/dvsapi/yearlist?tenant_id=602&callback={fn}
```
...
```
GET https://smart.discussvision.net/dvsapi/councilrd/all?tenant_id=602&year={year}&callback={fn}
```
    ...
    "schedules": [
      {
        ...
        "playlist": [
          {
            "playlist_id": "1",
            "speaker_img": "nishikata_norifumi.jpg",
            "speaker": "西方功文議員",
            "speaker_id": "8",
            "content": "■一般質問 1.新こども園建設計画について 2.湯沢川小水力発電事業について",
            "movie_name1": "nozawaonsen/2024/0311000101.mp4",
            "movie_name2": null,
            "movie_name3": null,
            "time_duration": null,
            "movie_released": "2",
            ...
            "vtt_name": null,
            "domain_vod_hls": "at.smartstream.ne.jp/vod/kaiken-257/_definst_/mp4:",
            "domain_vod_rtmp": "",
            "domain_live_hls": "",
            "domain_live_rtmp": "",
...
```
GET https://smart.discussvision.net/dvsapi/councilrd/search?tenant_id=602&keywords={keyword}&logical_op=AND&from={YYYY-MM-DD}&to={YYYY-MM-DD}&callback={fn}
```
...
```
GET https://smart.discussvision.net/dvsapi/minute/text?tenant_id=602&council_id={id}&schedule_id={id}&playlist_id={id}&callback={fn}
```

**注意**: 野沢温泉村議会の全データにおいて `minute_text` および `vtt_name` フィールドはいずれも空（`[]` / `null`）であり、会議録テキストは API 経由では取得できない。API を叩くとエラーコード 2004 が返る。
...
schedule の `label` には開催日と内容が含まれる（例: `03月11日　本会議`、`06月17日　本会議`）。
...
| フィールド | 説明 | 備考 |
| --- | --- | --- |
| `council_id` | 会議 ID | 全期間通しの連番（最古: 1、最新: 19） |
| `schedule_id` | 日程 ID | council 内での番号 |
| `playlist_id` | 発言順の連番 | schedule 内での 1 始まりの番号 |
| `speaker` | 発言者氏名 | 議員名が設定される（例: `西方功文議員`） |
| `speaker_id` | 発言者 ID | `0` は発言者なし（議事進行等） |
| `content` | 発言内容サマリー | 一般質問の質問テーマ等（例: `■一般質問 1.新こども園建設計画について`） |
| `movie_name1` | 動画ファイルパス | 例: `nozawaonsen/2024/0311000101.mp4` |
...
const res = await fetch(
  "https://smart.discussvision.net/dvsapi/yearlist?tenant_id=602&callback=cb"
);
...
for (const year of YEAR_LIST) {
  const res = await fetch(
    `https://smart.discussvision.net/dvsapi/councilrd/all?tenant_id=602&year=${year}&callback=cb`
  );
