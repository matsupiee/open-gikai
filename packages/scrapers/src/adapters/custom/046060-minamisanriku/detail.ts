...
import type { MinamisanrikuMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（星　喜美男君）　ただいまから → role=議長, name=星喜美男
 *   ○町長（佐藤　仁君）　お答えします → role=町長, name=佐藤仁
 *   ○７番（佐藤正明君）　質問します → role=議員, name=佐藤正明
 *   ○12番（菅原辰雄君）　... → role=議員, name=菅原辰雄
 */
...
/**
 * PDF テキストから開催日を抽出する。
 * PDF の表紙・タイトルページの「令和X年X月X日（曜日）」パターンを探す。
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[046060-minamisanriku] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = `minamisanriku_${fileName.replace(".pdf", "")}`;
