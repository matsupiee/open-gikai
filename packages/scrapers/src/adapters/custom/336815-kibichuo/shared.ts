/**
 * 吉備中央町議会 会議録 -- 共通ユーティリティ
 *
 * サイト: https://www.town.kibichuo.lg.jp/site/gikai/7847.html
 * 自治体コード: 336815
 */

export const BASE_ORIGIN = "https://www.town.kibichuo.lg.jp";

/** 会議録一覧ページ（全年度分が1ページにまとまっている） */
export const LIST_PAGE_URL = `${BASE_ORIGIN}/site/gikai/7847.html`;
...
export function convertHeadingToWesternYear(text: string): number | null {
  ...
  // 西暦年（例: "2024年(令和6年)"）
  const westernMatch = normalized.match(/^(\d{4})年/);
  if (westernMatch) {
    return Number(westernMatch[1]);
  }
