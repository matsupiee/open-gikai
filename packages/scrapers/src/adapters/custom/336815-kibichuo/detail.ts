...
export interface KibiChuoDetailParams {
  title: string;
  pdfUrl: string;
  meetingType: string;
  headingYear: number;
 ...
 *
 * 対応パターン:
 *   ○議長（西山宗弘君）　→ role=議長, name=西山宗弘
 *   ○町長（山本雅則君）　→ role=町長, name=山本雅則
 *   ○３番（我妻瑛子君）　→ role=議員, name=我妻瑛子
 *   ○総務課長（山本敦志君）　→ role=課長, name=山本敦志
 *   ○教育長（石井孝典君）　→ role=教育長, name=石井孝典
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[336815-kibichuo] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  // attachment ID を externalId に使う
  const attachmentIdMatch = params.pdfUrl.match(/\/attachment\/(\d+)\.pdf$/);
  const attachmentId = attachmentIdMatch?.[1] ?? params.pdfUrl;
  ...
  return {
    ...
    externalId: `kibichuo_${attachmentId}`,
    statements,
  };
}
