...
describe("extractHeldOnFromText", () => {
  it("令和の全角日付を変換する", () => {
    expect(extractHeldOnFromText("令和６年１２月４日")).toBe("2024-12-04");
  });

  it("令和の半角日付を変換する", () => {
    expect(extractHeldOnFromText("令和6年12月4日")).toBe("2024-12-04");
  });
...
describe("parseSpeaker", () => {
  it("議長パターンを抽出する（○マーカー）", () => {
    const result = parseSpeaker("○議長（西山宗弘君）　ただいまから会議を開きます。");
    expect(result.speakerName).toBe("西山宗弘");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("ただいまから会議を開きます。");
  });

  it("町長パターンを抽出する", () => {
    const result = parseSpeaker("○町長（山本雅則君）　お答えいたします。");
    expect(result.speakerName).toBe("山本雅則");
    expect(result.speakerRole).toBe("町長");
    expect(result.content).toBe("お答えいたします。");
  });

  it("番号付き議員パターンを抽出する（全角数字）", () => {
    const result = parseSpeaker("○３番（我妻瑛子君）　質問いたします。");
    expect(result.speakerName).toBe("我妻瑛子");
    expect(result.speakerRole).toBe("議員");
    expect(result.content).toBe("質問いたします。");
  });

  it("課長パターンを抽出する", () => {
    const result = parseSpeaker("○総務課長（山本敦志君）　お答えいたします。");
    expect(result.speakerName).toBe("山本敦志");
    expect(result.speakerRole).toBe("課長");
    expect(result.content).toBe("お答えいたします。");
  });

  it("教育長パターンを抽出する", () => {
    const result = parseSpeaker("○教育長（石井孝典君）　ご説明いたします。");
    expect(result.speakerName).toBe("石井孝典");
    expect(result.speakerRole).toBe("教育長");
    expect(result.content).toBe("ご説明いたします。");
  });

  it("副町長パターンを抽出する", () => {
    const result = parseSpeaker("○副町長（田中一郎君）　お答えします。");
  ...
  it("〇マーカー（U+3007）にも対応する", () => {
    const result = parseSpeaker("〇議長（西山宗弘君）　ただいまから会議を開きます。");
    expect(result.speakerName).toBe("西山宗弘");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("ただいまから会議を開きます。");
  });
  ...
  it("名前にスペースが含まれる場合スペースを除去する", () => {
    const result = parseSpeaker("○議長（西山　宗弘君）　開会します。");
    expect(result.speakerName).toBe("西山宗弘");
    expect(result.speakerRole).toBe("議長");
  });
...
describe("parseStatements", () => {
  it("○マーカーで分割して ParsedStatement 配列を生成する", () => {
    ...
    expect(result[0]!.speakerName).toBe("西山宗弘");
    ...
    expect(result[1]!.speakerName).toBe("我妻瑛子");
    expect(result[1]!.speakerRole).toBe("議員");

    expect(result[2]!.kind).toBe("answer");
    expect(result[2]!.speakerName).toBe("山本雅則");
  ...
  it("〇マーカー（U+3007）にも対応する", () => {
    const text = "〇議長（西山宗弘君）　ただいまから会議を開きます。";
    const result = parseStatements(text);
    expect(result).toHaveLength(1);
    expect(result[0]!.speakerRole).toBe("議長");
  });
  ...
  it("contentHash が SHA-256 形式で生成される", () => {
    const text = "○議長（西山宗弘君）　ただいまから会議を開きます。";
    const result = parseStatements(text);
