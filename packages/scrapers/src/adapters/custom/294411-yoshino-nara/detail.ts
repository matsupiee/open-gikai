...
export interface YoshinoDetailParams {
  title: string;
  heldOn: string | null;
  pdfUrl: string;
  meetingType: string;
 ...
 *
 * 対応パターン:
 *   ○議長（山本太郎）　 → role=議長, name=山本太郎
 *   ○町長（川端弘和）　 → role=町長, name=川端弘和
 *   ○３番（田中花子）　 → role=議員, name=田中花子
 *   ○総務課長（上田卓也）　 → role=課長, name=上田卓也
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[294411-yoshino-nara] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  const externalId = `yoshino_${fileName.replace(/\.pdf$/i, "")}`;
