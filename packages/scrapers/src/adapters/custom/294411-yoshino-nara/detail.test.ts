...
describe("parseSpeaker", () => {
  it("議長を正しくパースする", () => {
    const result = parseSpeaker("○議長（山本太郎） 本日の会議を開きます。");
  ...
  it("町長を正しくパースする", () => {
    const result = parseSpeaker("○町長（川端弘和） おはようございます。");
    expect(result.speakerName).toBe("川端弘和");
  ...
  it("番号付き議員を正しくパースする", () => {
    const result = parseSpeaker("○３番（田中花子） はい、議長。");
    expect(result.speakerName).toBe("田中花子");
    expect(result.speakerRole).toBe("議員");
    expect(result.content).toBe("はい、議長。");
  });

  it("課長（複合役職名）を正しくパースする", () => {
    const result = parseSpeaker("○総務課長（上田卓也） ご報告いたします。");
    expect(result.speakerName).toBe("上田卓也");
    expect(result.speakerRole).toBe("課長");
    expect(result.content).toBe("ご報告いたします。");
  });

  it("教育長を正しくパースする", () => {
    const result = parseSpeaker("○教育長（西村欣也） 答弁申し上げます。");
    expect(result.speakerName).toBe("西村欣也");
    expect(result.speakerRole).toBe("教育長");
    expect(result.content).toBe("答弁申し上げます。");
  });

  it("副議長を正しくパースする", () => {
    const result = parseSpeaker("○副議長（鈴木一郎） 議事を進めます。");
...
describe("parseStatements", () => {
  it("○ マーカーで発言を正しく分割する", () => {
    const text = `
○議長（山本太郎） 本日の会議を開きます。
○町長（川端弘和） おはようございます。令和６年第３回定例会の開催に当たりまして。
○３番（田中花子） はい、議長。
    `.trim();
    ...
    expect(statements[1]!.speakerName).toBe("川端弘和");
    expect(statements[1]!.speakerRole).toBe("町長");
    expect(statements[1]!.kind).toBe("answer");

    expect(statements[2]!.speakerName).toBe("田中花子");
