...
export const BASE_ORIGIN = "https://www.town.yokohama.lg.jp";
export const LIST_URL = `${BASE_ORIGIN}/index.cfm/10,0,47,html`;
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[yokohama-aomori] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(`[yokohama-aomori] fetchPage failed: ${url}`, e);
    return null;
  }
}
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[yokohama-aomori] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(`[yokohama-aomori] fetchBinary failed: ${url}`, e);
    return null;
  }
}
