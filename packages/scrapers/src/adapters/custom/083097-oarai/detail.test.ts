...
describe("parseSpeaker", () => {
  ...
  it("副町長をパースする", () => {
    const result = parseSpeaker("○副町長（山田次郎君） ご説明いたします。");
  ...
  it("課長をパースする（複合役職名）", () => {
    const result = parseSpeaker("○福祉課長（高橋四郎君） ご説明申し上げます。");
  ...
  it("半角数字番号付き議員をパースする", () => {
    const result = parseSpeaker("○1番（山田花子君） 質問いたします。");
  ...
  it("全角数字番号付き議員をパースする", () => {
    const result = parseSpeaker("○１番（山田花子君） 質問いたします。");
    expect(result.speakerName).toBe("山田花子");
    expect(result.speakerRole).toBe("議員");
    expect(result.content).toBe("質問いたします。");
  });
  ...
  it("副議長をパースする", () => {
    const result = parseSpeaker("○副議長（伊藤五郎君） 暫時休憩いたします。");
  ...
  it("事務局長をパースする", () => {
    const result = parseSpeaker("○議会事務局長（渡辺六郎君） それでは朗読いたします。");
...
describe("parseHeldOn", () => {
  it("「開 会」パターンの開催日を抽出する", () => {
    const text = "令和6年12月3日 開 会 令和6年12月17日 閉 会";
    expect(parseHeldOn(text)).toBe("2024-12-03");
  });
...
describe("parseDateFromDayLabel", () => {
  it("dayLabel とタイトルから日付を生成する", () => {
    expect(parseDateFromDayLabel("12月3日", "令和6年第4回議会定例会")).toBe("2024-12-03");
  });

  it("令和元年の処理ができる", () => {
    expect(parseDateFromDayLabel("9月10日", "令和元年第3回議会定例会")).toBe("2019-09-10");
  });

  it("平成年の処理ができる", () => {
    expect(parseDateFromDayLabel("12月5日", "平成30年第4回議会定例会")).toBe("2018-12-05");
  });

  it("全角数字の dayLabel を処理できる", () => {
    expect(parseDateFromDayLabel("１２月３日", "令和6年第4回議会定例会")).toBe("2024-12-03");
  });

  it("日付情報がない場合は null を返す", () => {
    expect(parseDateFromDayLabel("目次", "令和6年第4回議会定例会")).toBeNull();
  });

  it("タイトルに年情報がない場合は null を返す", () => {
    expect(parseDateFromDayLabel("12月3日", "第4回議会定例会")).toBeNull();
  });
});
