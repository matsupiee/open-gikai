...
import type { TakaharuMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（松元 重則君） → role=議長, name=松元 重則
 *   ○町長（溝口 誠二君） → role=町長, name=溝口 誠二
 *   ○○番（田中 勇君） → role=議員, name=田中 勇
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[453617-takaharu] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  // PDF URL からアタッチメント ID を externalId として利用
