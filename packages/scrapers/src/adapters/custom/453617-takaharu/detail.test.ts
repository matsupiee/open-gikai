...
describe("parseSpeaker", () => {
  it("議長（君付き）を正しくパースする", () => {
    const result = parseSpeaker("○議長（松元 重則君） ただいまから本日の会議を開きます。");
    expect(result.speakerName).toBe("松元 重則");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("ただいまから本日の会議を開きます。");
  });

  it("町長を正しくパースする", () => {
    const result = parseSpeaker("○町長（溝口 誠二君） お答えいたします。");
    expect(result.speakerName).toBe("溝口 誠二");
  ...
  it("副議長を正しくパースする", () => {
    const result = parseSpeaker("○副議長（鈴木 三郎君） 議事を進めます。");
  ...
  it("副委員長を正しくパースする", () => {
    const result = parseSpeaker("○副委員長（山口 花子君） 委員会を開会します。");
    expect(result.speakerName).toBe("山口 花子");
...
describe("parseStatements", () => {
  it("○ マーカーで発言を分割する", () => {
    const text =
      "○議長（松元 重則君） ただいまから本日の会議を開きます。 " +
      "○町長（溝口 誠二君） 政務報告を申し上げます。 " +
      "○５番（田中 勇君） 質問いたします。";
    ...
    expect(statements[0]!.speakerName).toBe("松元 重則");
    expect(statements[0]!.speakerRole).toBe("議長");
    expect(statements[0]!.kind).toBe("remark");
    expect(statements[0]!.content).toBe("ただいまから本日の会議を開きます。");

    expect(statements[1]!.speakerName).toBe("溝口 誠二");
  ...
  it("contentHash が SHA-256 形式で生成される", () => {
    const text = "○議長（松元 重則君） ただいまから会議を開きます。";
  ...
  it("空の content はスキップする", () => {
    const text = "前置きテキスト ○議長（松元 重則君） 会議を開きます。";
  ...
  it("課長の発言は answer に分類される", () => {
    const text = "○総務課長（橋口 誠二君） 説明いたします。";
