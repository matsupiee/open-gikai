...
import type { GojomeMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（佐藤太郎君）　→ role=議長, name=佐藤太郎
 *   ○町長（山田一郎君）　→ role=町長, name=山田一郎
 *   ○12番（鈴木花子君）  → role=議員, name=鈴木花子
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `gojome_${idKey}` : null;
