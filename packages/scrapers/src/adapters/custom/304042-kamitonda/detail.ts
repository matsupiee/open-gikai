...
export interface KamitondaDetailParams {
  title: string;
  heldOn: string | null;
  pdfUrl: string;
  meetingType: string;
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[304042-kamitonda] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  return {
    ...
    externalId: `kamitonda_${params.fileName.replace(/\.pdf$/i, "")}`,
    statements,
  };
}
