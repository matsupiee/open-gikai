 ...
 *   ○12番（山村　明議員）　質問します。
 *   ○畑山淳一企画調整部長　お答えいたします。
 *   ○細谷　晃事務局長　お名前をお呼びいたします。
 */

import { createHash } from "node:crypto";
import { extractText, getDocumentProxy } from "unpdf";
import type { MeetingData, ParsedStatement } from "../../../utils/types";
import type { YonezawaMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○島軒純一議長　…         → role=議長, name=島軒純一
 *   ○近藤洋介市長　…         → role=市長, name=近藤洋介
 *   ○12番（山村　明議員）　… → role=議員, name=山村　明
 *   ○畑山淳一企画調整部長　… → role=部長, name=畑山淳一
 */
export function parseSpeaker(text: string): {
  ...
  // パターン3: {name}{role} content (e.g., 島軒純一議長 ...)
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[062022-yonezawa] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `yonezawa_${idKey}` : null;
