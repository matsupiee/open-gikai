...
describe("extractExternalIdKey", () => {
  it("ハイフン区切りのファイル名からキーを抽出する", () => {
    expect(
      extractExternalIdKey(
        "https://www.town.gonohe.aomori.jp/chosei/gikai/gonohe-gikai-kaigiroku18-13.pdf",
      ),
    ).toBe("18-13");
  });

  it("アンダースコア区切りのファイル名からキーを抽出する", () => {
    expect(
      extractExternalIdKey(
  ...
  it("マッチしないURLはnullを返す", () => {
    expect(extractExternalIdKey("https://example.com/some-doc.pdf")).toBeNull();
  });
});
