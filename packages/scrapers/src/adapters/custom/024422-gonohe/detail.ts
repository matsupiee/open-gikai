...
import type { GonoheMeeting } from "./list";
 ...
 * 五戸町議会の PDF は冒頭に目次と議案件名・出席議員名簿等があり、
 ...
 * e.g., "gonohe-gikai-kaigiroku18-13.pdf" → "18-13"
 *       "gonohe-gikai-kaigiroku_16-9.pdf" → "16-9"
 */
export function extractExternalIdKey(pdfPath: string): string | null {
  const match = pdfPath.match(/kaigiroku[_-]?(\d+-\d+)\.pdf$/i);
