...
export interface KitahiroshimaDetailParams {
  title: string;
  heldOn: string;
  pdfUrl: string;
  meetingType: string;
 ...
 *
 * 対応パターン:
 *   ○議長（湊俊文）　→ role=議長, name=湊俊文
 *   ○町長（箕野博司）　→ role=町長, name=箕野博司
 *   ○３番（花岡元治君）　→ role=議員, name=花岡元治
 *   ○建設課長（竹下秀樹）　→ role=建設課長, name=竹下秀樹
 */
export function parseSpeaker(text: string): {
  ...
  if (match) {
    ...
    const rawName = match[2]!.replace(/[\s　]+/g, "").replace(/君$|様$/, "").trim();
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[343692-kitahiroshima-town] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  // externalId: attachment ID を使う
  const idMatch = params.pdfUrl.match(/\/attachment\/(\d+)\.pdf$/);
  const externalId = idMatch
    ? `kitahiroshima_town_${idMatch[1]}`
