...
export const BASE_ORIGIN = "https://www.town.oirase.aomori.jp";
export const LIST_URL = `${BASE_ORIGIN}/site/gikai/list19-60.html`;
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[oirase] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(`[oirase] fetchPage failed: ${url}`, e);
    return null;
  }
}
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[oirase] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(`[oirase] fetchBinary failed: ${url}`, e);
    return null;
  }
}
