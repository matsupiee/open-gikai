...
export interface ReihokuDetailParams {
  title: string;
  pdfUrl: string;
  meetingType: string;
  year: number;
...
export function estimateHeldOn(
  ...
  const sessionMatch = title.match(/第(\d+)回(定例会|臨時会)/);
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[435317-reihoku] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  return {
    ...
    externalId: `reihoku_${params.year}_${params.title}`,
    statements,
  };
}
