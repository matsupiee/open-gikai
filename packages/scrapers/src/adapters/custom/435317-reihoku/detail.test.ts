...
describe("estimateHeldOn", () => {
  it("第1回定例会は3月と推定する", () => {
    expect(estimateHeldOn("令和6年第1回定例会", 2024)).toBe("2024-03-01");
  });

  it("第2回定例会は6月と推定する", () => {
    expect(estimateHeldOn("令和6年第2回定例会", 2024)).toBe("2024-06-01");
  });

  it("第3回定例会は9月と推定する", () => {
    expect(estimateHeldOn("令和6年第3回定例会", 2024)).toBe("2024-09-01");
  });

  it("第4回定例会は12月と推定する", () => {
    expect(estimateHeldOn("令和6年第4回定例会", 2024)).toBe("2024-12-01");
  });

  it("臨時会は null を返す", () => {
    expect(estimateHeldOn("令和6年第1回臨時会", 2024)).toBeNull();
  });

  it("回次がマッピング外の場合は null を返す", () => {
    expect(estimateHeldOn("令和6年第5回定例会", 2024)).toBeNull();
  });
