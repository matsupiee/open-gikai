 ...
 * 天龍村議会 — detail フェーズ
 *
 * 議会活動記録 PDF をダウンロードしてテキストを抽出し、
 * ○ マーカーで発言を分割して ParsedStatement 配列を生成する。
 *
 * 会議録本文は PDF のみで提供されているため、
 * PDF ダウンロード → テキスト抽出 → 発言分割のパイプラインを使用する。
 */

import { createHash } from "node:crypto";
import { extractText, getDocumentProxy } from "../../../utils/pdf";
import type { MeetingData, ParsedStatement } from "../../../utils/types";
import type { TenryuActivityRecord } from "./list";
...
export function parseSpeaker(text: string): {
  ...
  if (answerMatch) {
    const rolePart = answerMatch[1]!.trim();
    ...
    return { speakerName: null, speakerRole: rolePart || null, content };
  }
...
export async function fetchMeetingData(
  ...
  const externalId = extractExternalIdKey(meeting.pdfUrl);
