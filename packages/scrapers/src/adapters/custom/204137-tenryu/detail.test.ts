...
describe("parseSpeaker", () => {
  ...
  it("答弁（役職）パターンを解析する", () => {
    ...
    expect(result.speakerRole).toBe("村長");
    expect(result.content).toBe("予算については検討します。");
  });
...
describe("extractExternalIdKey", () => {
  it("WordPress アップロードパスから ID を抽出する", () => {
    const result = extractExternalIdKey(
      "https://www.vill-tenryu.jp/wp-content/uploads/2025/02/abc123def456.pdf"
    );
    expect(result).toBe("tenryu_abc123def456");
  });

  it("ハッシュ値ファイル名から ID を抽出する", () => {
    const result = extractExternalIdKey(
      "https://www.vill-tenryu.jp/wp-content/uploads/2023/07/xyz789hash.pdf"
    );
    expect(result).toBe("tenryu_xyz789hash");
  });

  it("PDF URL でない場合は null を返す", () => {
    expect(extractExternalIdKey("https://example.com/page")).toBeNull();
  });
});
