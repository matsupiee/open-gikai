...
import type { MinamiizuMeeting } from "./list";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[223042-minamiizu] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  // ファイル名をキーとして externalId を生成
  const stem = meeting.filename.replace(/\.pdf$/i, "");
  const externalId = `minamiizu_${stem}`;

  // heldOn が null の場合は解析失敗扱い（"1970-01-01" 禁止）
