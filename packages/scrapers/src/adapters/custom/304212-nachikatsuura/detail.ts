...
export interface NachikatsuuraDetailParams {
  title: string;
  heldOn: string | null;
  pdfUrl: string;
  meetingType: string;
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[304212-nachikatsuura] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  params: NachikatsuuraDetailParams,
  ...
  return {
    ...
    externalId: `nachikatsuura_${params.fileName.replace(/\.pdf$/i, "")}`,
    statements,
  };
}
