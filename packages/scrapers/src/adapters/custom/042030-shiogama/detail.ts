/**
 * 塩竈市議会（宮城県）会議録 — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、発言を解析する。
 *
 * 発言フォーマット（PDF から抽出したテキスト）:
 *   ○議長（氏名君）　ただいまから会議を開きます。
 *   ○市長（氏名君）　お答えいたします。
 *   ○1番（氏名君）　質問いたします。
 */
...
/** fetchMeetingData に渡す会議録パラメータ */
export interface ShiogamaMeetingParams {
  pdfUrl: string;
  title: string;
  yearHeading: string;
  typeHeading: string;
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[042030-shiogama] PDF テキスト抽出失敗: ${pdfUrl}`,
