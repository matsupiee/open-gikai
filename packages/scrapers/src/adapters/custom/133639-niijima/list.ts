 ...
 *
 * API エンドポイント:
 *   GET /dvsapi/councilrd/all?tenant_id=188&year={year}
 *
 ...
 */
export function parseCouncilList(data: unknown): NijimaRecord[] {
  if (!Array.isArray(data)) return [];

  const results: NijimaRecord[] = [];
  ...
  for (const council of data as DvsCouncil[]) {
    ...
    const sourceUrl = `${BASE_ORIGIN}/smart/tenant/niijima/WebView/rd/council_1.html`;
 ...
 */
export async function fetchDocumentList(year: number): Promise<NijimaRecord[]> {
  const url = `${BASE_ORIGIN}/dvsapi/councilrd/all?tenant_id=${TENANT_ID}&year=${year}`;
  const data = await fetchJson(url);
  if (!data) {
    console.warn(`[133639-niijima] No data returned for year=${year}`);
    return [];
  }
