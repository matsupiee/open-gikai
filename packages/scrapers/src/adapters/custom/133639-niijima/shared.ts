...
/**
 * JSON 文字列に含まれる制御文字（改行以外）をサニタイズする。
 * DiscussVision API のレスポンスには制御文字が含まれる場合がある。
 */
export function sanitizeJson(text: string): string {
  return text.replace(/[\x00-\x1f\x7f]/g, (c) => (c === "\n" ? "\n" : ""));
}
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[133639-niijima] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(
      `[133639-niijima] fetchPage error: ${url}`,
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[133639-niijima] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(
      `[133639-niijima] fetchBinary error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
...
export async function fetchJson(url: string): Promise<unknown> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[133639-niijima] fetchJson failed: ${url} status=${res.status}`);
      return null;
    }
    ...
    console.warn(
      `[133639-niijima] fetchJson error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
