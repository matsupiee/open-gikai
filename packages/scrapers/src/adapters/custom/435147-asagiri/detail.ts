/**
 * あさぎり町議会 会議録 — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、○ マーカーで発言を分割して
 * ParsedStatement 配列を生成する。
 *
 * あさぎり町の会議録 PDF では発言者が ○ マーカーで区切られている想定。
 * フォーマット例:
 *   ○議長（○○○○君）　ただいまから会議を開きます。
 *   ○１番（○○○○君）　質問いたします。
 *   ○町長（○○○○君）　お答えいたします。
...
import type { AsagiriMeeting } from "./list";
import {
  buildDetailUrl,
  buildExternalId,
  detectMeetingType,
...
export function parseSpeaker(text: string): {
  ...
  // パターン2: {role}（{name}君|議員|様）content (e.g., 議長（小町明夫君）)
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[435147-asagiri] PDF 取得失敗: ${pdfUrl}`,
