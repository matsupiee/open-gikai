...
describe("parseSpeaker", () => {
  ...
  it("番号付き議員パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("森澤一郎");
  ...
  it("名前に空白を含む場合は除去される", () => {
    ...
    expect(result.speakerName).toBe("中村太郎");
    expect(result.speakerRole).toBe("議長");
  });
...
describe("parseHeldOn", () => {
  it("PDF リンクテキストから日付を抽出する（優先）", () => {
    const text = "令和7年3月4日\n立科町議会定例会\n○議長（田中君） 開会します。";
    const linkText = "3月4日本会議開会・町長招集挨拶 (PDFファイル: 711.8KB)";
    expect(parseHeldOn(text, 2025, linkText)).toBe("2025-03-04");
  });

  it("PDF リンクテキストがない場合は PDF 本文から抽出する", () => {
    const text = "令和7年6月10日\n立科町議会定例会\n○議長（田中君） 開会します。";
    expect(parseHeldOn(text, 2025)).toBe("2025-06-10");
  });

  it("全角数字の日付を PDF リンクテキストから抽出する", () => {
    const text = "";
    const linkText = "3月19日委員長報告・質疑・討論・採決 (PDFファイル: 698.3KB)";
    expect(parseHeldOn(text, 2025, linkText)).toBe("2025-03-19");
  });

  it("全角数字の日付を本文から抽出する", () => {
    const text = "令和７年６月１０日\n立科町議会定例会";
    expect(parseHeldOn(text, 2025)).toBe("2025-06-10");
  });

  it("平成の日付を本文から抽出する", () => {
    const text = "平成24年3月1日\n立科町議会臨時会\n○議長（鈴木君） 開会します。";
    expect(parseHeldOn(text, 2012)).toBe("2012-03-01");
  });

  it("日付が見つからない場合はnullを返す", () => {
    expect(parseHeldOn("立科町議会会議録", 2025)).toBeNull();
    expect(parseHeldOn("", 2025)).toBeNull();
  });
});
...
`;
    ...
    expect(statements[1]!.speakerName).toBe("森澤一郎");
