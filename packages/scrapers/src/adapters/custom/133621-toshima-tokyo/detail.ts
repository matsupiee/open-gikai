...
import type { ToshimaMeeting } from "./list";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[133621-toshima-tokyo] PDF 取得失敗: ${pdfUrl}`,
...
/**
 * PDF パスから externalId を生成する。
 * e.g., "/fs/2/7/1/6/9/1/_/___1___.pdf" → "toshima_2_7_1_6_9_1"
 */
function extractExternalId(pdfUrl: string): string | null {
  const match = pdfUrl.match(/\/fs\/([\d/]+)\/_\//);
  if (!match) return null;
  const pathPart = match[1]!.replace(/\//g, "_");
  return `toshima_${pathPart}`;
}

/**
 * 会議情報から開催日付を推定する（月は不明なため年のみ）。
 * 利島村の PDF には月情報が HTML リンクテキストに含まれないため、
 * 年の 1 月 1 日として記録する。
 */
export function buildHeldOn(year: number): string {
  return `${year}-01-01`;
}

/**
 * PDF をダウンロード・テキスト抽出し、MeetingData に変換する。
 * テキスト抽出が空の場合は null を返す。
 */
