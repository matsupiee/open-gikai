...
describe("parseSpeaker", () => {
  it("議長（名前君）パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("田口誠");
  ...
  it("村長（名前君）パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("今井俊郎");
  ...
  it("番号議員パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("田口義之");
  ...
  it("名前に空白を含む場合は除去される", () => {
    const result = parseSpeaker("○村長（今井　俊郎君）　答弁します。");
    expect(result.speakerName).toBe("今井俊郎");
  });
...
`;
    ...
    expect(statements[0]!.speakerName).toBe("田口誠");
    expect(statements[0]!.speakerRole).toBe("議長");

    expect(statements[1]!.kind).toBe("question");
    expect(statements[1]!.speakerName).toBe("田口義之");
    expect(statements[1]!.speakerRole).toBe("議員");

    expect(statements[2]!.kind).toBe("answer");
    expect(statements[2]!.speakerName).toBe("今井俊郎");
...
○３番（田口義之君）　質問です。`;
  ...
  it("ト書き（登壇）はスキップする", () => {
    const text = `○議長（田口　誠君）　ただいまから会議を開きます。
（３番　田口義之君登壇）
○３番（田口義之君）　質問があります。`;
