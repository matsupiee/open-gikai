/**
 * 和気町議会 -- ScraperAdapter 実装
 *
 * サイト: https://www.town.wake.lg.jp/
 * 自治体コード: 333468
 *
 * 和気町は PDF ファイルで会議録を公開しており、
...
import { buildMeetingData, type WakeDetailParams } from "./detail";
...
export const adapter: ScraperAdapter = {
  ...
  async fetchList({ year }): Promise<ListRecord[]> {
    ...
    return documents.map((doc) => ({
      ...
      } satisfies WakeDetailParams,
    }));
  },

  async fetchDetail({ detailParams, municipalityId }) {
    const params = detailParams as unknown as WakeDetailParams;
