/**
 * 錦町議会 会議録 — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、○ マーカーで発言を分割して
 * ParsedStatement 配列を生成する。
 *
 * 錦町の会議録 PDF では発言者が ○ マーカーで区切られている想定。
 * フォーマット例:
 *   ○議長（○○○○君）　ただいまから会議を開きます。
 *   ○１番（○○○○君）　質問いたします。
 *   ○町長（○○○○君）　お答えいたします。
...
import { buildYearPageUrl, detectMeetingType, fetchBinary } from "./shared";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[435015-nishiki] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  return {
    ...
    sourceUrl: buildYearPageUrl(meeting.kijiId),
