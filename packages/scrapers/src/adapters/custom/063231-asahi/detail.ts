...
export function parseStatements(text: string): ParsedStatement[] {
  ...
  const normalized = targetText.replace(
    /(?=(?:教\s*育\s*文\s*化\s*課\s*長|課\s*長\s*補\s*佐|生\s*涯\s*学\s*習\s*係\s*長|事\s*務\s*局\s*次\s*長|事\s*務\s*局\s*長|指\s*導\s*主\s*事|副\s*議\s*長|教\s*育\s*長|課\s*長|係\s*長|議\s*長|主\s*幹|補\s*佐|主\s*査|\d+\s*番\s*委\s*員)\s*[-－–]\s)/g,
    "\n"
  );
