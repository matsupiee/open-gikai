...
export const BASE_ORIGIN = "https://www.town.saka.lg.jp";
export const WP_API_URL = `${BASE_ORIGIN}/wp-json/wp/v2/posts/1195`;
...
export async function fetchJson(url: string): Promise<unknown | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`fetchJson failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.json();
  } catch (e) {
    console.warn(`fetchJson error: ${url}`, e instanceof Error ? e.message : e);
    return null;
  }
}
