...
export interface TsugaruDetailParams {
  title: string;
  year: number;
  heldOn: string | null;
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[022098-tsugaru] PDF 取得失敗: ${pdfUrl}`,
 ...
 * e.g., "/material/files/group/27/R0703teireikai.pdf" → "R0703teireikai"
 */
...
export async function fetchMeetingData(
  ...
  const heldOn = params.heldOn ?? parseJapaneseDate(text);
  ...
  const externalId = idKey ? `tsugaru_${idKey}` : null;
