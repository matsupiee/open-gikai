...
describe("parseJapaneseDate", () => {
  it("令和の日付を西暦に変換する", () => {
    expect(parseJapaneseDate("令和7年3月10日")).toBe("2025-03-10");
    expect(parseJapaneseDate("令和6年9月5日")).toBe("2024-09-05");
  });

  it("平成の日付を西暦に変換する", () => {
    expect(parseJapaneseDate("平成25年6月1日")).toBe("2013-06-01");
  });

  it("全角数字を含む日付を変換する", () => {
    expect(parseJapaneseDate("令和７年３月１０日")).toBe("2025-03-10");
  });
  ...
  it("テキスト中の日付を抽出する", () => {
    expect(
      parseJapaneseDate(
        "開議　令和7年3月10日（月曜日）午前10時00分"
      )
    ).toBe("2025-03-10");
  });

  it("日付が含まれない場合は null を返す", () => {
    expect(parseJapaneseDate("第1回定例会本会議")).toBeNull();
