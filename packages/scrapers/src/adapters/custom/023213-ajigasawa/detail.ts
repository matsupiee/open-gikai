...
import type { AjigasawaMeeting } from "./list";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[023213-ajigasawa] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = meeting.fileKey
    ? `ajigasawa_${meeting.fileKey}`
    : null;
