...
describe("parseHeldOn", () => {
  it("令和の年月日を YYYY-MM-DD に変換する", () => {
    expect(parseHeldOn("令和7年3月3日")).toBe("2025-03-03");
    expect(parseHeldOn("令和6年12月13日")).toBe("2024-12-13");
  });

  it("令和元年に対応する", () => {
    expect(parseHeldOn("令和元年9月9日")).toBe("2019-09-09");
  });

  it("平成の年月日を変換する", () => {
    expect(parseHeldOn("平成31年3月1日")).toBe("2019-03-01");
  });

  it("平成元年に対応する", () => {
    expect(parseHeldOn("平成元年4月1日")).toBe("1989-04-01");
  });

  it("全角数字を正規化して変換する", () => {
    expect(parseHeldOn("令和６年１２月１３日")).toBe("2024-12-13");
  });

  it("日付がない場合は null を返す", () => {
    expect(parseHeldOn("マッチしないテキスト")).toBeNull();
  });
});
...
describe("parseSpeaker", () => {
  ...
  it("副委員長は 委員長 より先にマッチする", () => {
    const result = parseSpeaker("○副委員長（山田君）　暫時休憩します。");
...
describe("parseStatements", () => {
  ...
  it("contentHash は content の sha256 ハッシュ", () => {
    const text = "○議長（山田君）　テスト発言。";
    const result = parseStatements(text);

    expect(result).toHaveLength(1);
    expect(result[0]!.contentHash).toBe(
      createHash("sha256").update("テスト発言。").digest("hex"),
    );
  });
});
