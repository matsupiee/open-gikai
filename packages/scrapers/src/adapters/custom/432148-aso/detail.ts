...
import type { AsoMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（北里秀和君）　→ role=議長, name=北里秀和
 *   ○市長（佐藤義興君）　→ role=市長, name=佐藤義興
 *   ○３番（髙宮辰郎君）　→ role=議員, name=髙宮辰郎
 *   ○教育部長（田上修一君）→ role=部長, name=田上修一
 */
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `aso_${idKey}` : null;
