...
import { buildHeldOn, classifyKind, parseSpeaker, parseStatements } from "./detail";

describe("parseSpeaker", () => {
  it("議長パターンを抽出する", () => {
    const result = parseSpeaker("○議長（田中一郎君）　ただいまから会議を開きます。");
  ...
  it("副議長パターンを抽出する", () => {
    const result = parseSpeaker("○副議長（山田次郎君）　休憩いたします。");
  ...
  it("町長パターンを抽出する", () => {
    const result = parseSpeaker("○町長（鈴木三郎君）　お答えいたします。");
  ...
  it("副町長パターンを抽出する", () => {
    const result = parseSpeaker("○副町長（佐藤四郎君）　ご説明いたします。");
  ...
  it("教育長パターンを抽出する", () => {
    const result = parseSpeaker("○教育長（高橋五郎君）　お答えいたします。");
  ...
  it("番号付き議員パターンを抽出する", () => {
    const result = parseSpeaker("○２番（伊藤六郎君）　質問いたします。");
  ...
  it("部長パターンを抽出する", () => {
    const result = parseSpeaker("○総務部長（渡辺七郎君）　ご報告いたします。");
  ...
  it("名前に全角スペースがある場合に除去する", () => {
    const result = parseSpeaker("○町長（鈴木　三郎君）　お答えします。");
    expect(result.speakerName).toBe("鈴木三郎");
    expect(result.speakerRole).toBe("町長");
  });
  ...
  it("委員長パターンを抽出する", () => {
    const result = parseSpeaker("○委員長（松田八郎君）　委員会を開会いたします。");
    expect(result.speakerName).toBe("松田八郎");
    expect(result.speakerRole).toBe("委員長");
    expect(result.content).toBe("委員会を開会いたします。");
  });

  it("副委員長パターンを抽出する（副委員長 > 委員長 の順序確認）", () => {
    const result = parseSpeaker("○副委員長（中村九郎君）　暫時休憩いたします。");
    expect(result.speakerName).toBe("中村九郎");
...
describe("buildHeldOn", () => {
  it("定例会第1回 → 3月", () => {
    expect(buildHeldOn(2024, 1, "plenary")).toBe("2024-03-01");
  });

  it("定例会第2回 → 6月", () => {
    expect(buildHeldOn(2024, 2, "plenary")).toBe("2024-06-01");
  });

  it("定例会第3回 → 9月", () => {
    expect(buildHeldOn(2024, 3, "plenary")).toBe("2024-09-01");
  });

  it("定例会第4回 → 12月", () => {
    expect(buildHeldOn(2024, 4, "plenary")).toBe("2024-12-01");
  });

  it("定例会第5回（不明）は null を返す", () => {
    expect(buildHeldOn(2024, 5, "plenary")).toBeNull();
  });

  it("臨時会は null を返す", () => {
    expect(buildHeldOn(2024, 1, "extraordinary")).toBeNull();
  });
});
...
describe("parseStatements", () => {
  it("○マーカーで分割して ParsedStatement 配列を生成する", () => {
    ...
    expect(result[1]!.speakerName).toBe("伊藤六郎");
    expect(result[1]!.speakerRole).toBe("議員");

    expect(result[2]!.kind).toBe("answer");
    expect(result[2]!.speakerName).toBe("鈴木三郎");
  ...
  it("contentHash が SHA-256 ハッシュ（64文字）で生成される", () => {
