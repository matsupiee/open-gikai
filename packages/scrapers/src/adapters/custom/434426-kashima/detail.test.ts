...
import { classifyKind, parseSpeaker, parseStatements, parsePdfLinks } from "./detail";
...
describe("parseSpeaker", () => {
  ...
  it("副町長の発言を抽出する", () => {
    const result = parseSpeaker("○副町長（伊藤五郎君）　ご説明いたします。");
  ...
  it("教育長の発言を抽出する", () => {
    const result = parseSpeaker("○教育長（中村四郎君）　ご報告いたします。");
...
describe("parsePdfLinks", () => {
  it("/dl?q= リンクを抽出する", () => {
    const html = `
      <div>
        <a href="/dl?q=39813_filelib_918ab7be7640c1103a74bf1ca673331a.pdf">令和7年4月(12月)会議録(PDF 約785KB)</a>
        <a href="/dl?q=12345_filelib_abc123def456.pdf">令和7年第2回会議録(PDF 約500KB)</a>
      </div>
    `;
    ...
    expect(links[0]).toBe(
      "https://www.town.kumamoto-kashima.lg.jp/dl?q=39813_filelib_918ab7be7640c1103a74bf1ca673331a.pdf",
    );
    expect(links[1]).toBe(
      "https://www.town.kumamoto-kashima.lg.jp/dl?q=12345_filelib_abc123def456.pdf",
    );
  });

  it("重複する URL を除外する", () => {
    const html = `
      <a href="/dl?q=39813_filelib_918ab7be7640c1103a74bf1ca673331a.pdf">会議録1</a>
      <a href="/dl?q=39813_filelib_918ab7be7640c1103a74bf1ca673331a.pdf">会議録1（重複）</a>
    `;
  ...
  it("PDF リンクがない場合は空配列を返す", () => {
    const html = "<div><a href='/other/page.html'>他のリンク</a></div>";
