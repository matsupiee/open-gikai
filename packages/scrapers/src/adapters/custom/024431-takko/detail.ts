 ...
 *
 * 注意: 田子町の PDF は議案審議結果一覧（会議録ではない）のため、
 * 発言者パースが適用できない場合がある。テキスト全体を単一の remark として扱う。
 */
 ...
 *
 * ○ マーカーで発言を分割する。
 * ○ マーカーがない場合はテキスト全体を単一の remark として扱う。
 */
...
/**
 * PDF テキストから開催日を抽出する。
 * PDF 冒頭部分に「令和 7 年 12 月 25 日」のようなパターンが含まれる。
 * 空白が混在するため柔軟にマッチさせる。
 */
...
export async function fetchMeetingData(
  ...
  const externalId = filenameMatch ? `takko_${filenameMatch[1]}` : null;
