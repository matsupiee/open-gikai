...
describe("parseSpeaker", () => {
  ...
  it("副委員長パターンを解析する（委員長より先にマッチする）", () => {
    const result = parseSpeaker(
      "○副委員長（鈴木六郎）　審議を続けます。"
    );
    expect(result.speakerName).toBe("鈴木六郎");
    expect(result.speakerRole).toBe("副委員長");
    expect(result.content).toBe("審議を続けます。");
  });
...
describe("classifyKind", () => {
  ...
  it("事務局次長は answer", () => {
    expect(classifyKind("事務局次長")).toBe("answer");
  });
  ...
  it("複合役職（企画部長など）は answer", () => {
    // parseSpeaker で rolePart.endsWith("部長") → speakerRole = "部長" なので classifyKind("部長") = answer
    expect(classifyKind("部長")).toBe("answer");
  });
});
...
`;
  ...
  it("contentHash は SHA-256 の 64 文字 hex", () => {
    const statements = parseStatements("○3番（佐藤花子）　一般質問をします。");
