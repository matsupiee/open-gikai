...
import type { YuasaMeeting } from "./list";
...
export async function fetchMeetingData(
  ...
  // PDF の数値 ID から externalId を生成
  const pdfIdMatch = meeting.pdfUrl.match(/\/uploaded\/attachment\/(\d+)\.pdf$/);
  const externalId = pdfIdMatch ? `yuasa_${pdfIdMatch[1]}` : null;
