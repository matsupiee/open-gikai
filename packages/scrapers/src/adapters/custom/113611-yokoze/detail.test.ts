...
describe("parseSpeaker", () => {
  ...
  it("番号付き議員（君）を正しくパースする", () => {
    const result = parseSpeaker("○3番（高橋次郎君）　質問いたします。");
    expect(result.speakerName).toBe("高橋次郎");
    expect(result.speakerRole).toBe("議員");
    expect(result.content).toBe("質問いたします。");
  });

  it("番号付き議員（議員）を正しくパースする", () => {
    const result = parseSpeaker("○5番（田中三郎議員）　質問です。");
...
`;
  ...
  it("全角数字の番号付き議員も処理する", () => {
    const text = `○３番（田中次郎君）　一般質問を行います。`;
    const statements = parseStatements(text);

    expect(statements.length).toBe(1);
    expect(statements[0]!.speakerName).toBe("田中次郎");
