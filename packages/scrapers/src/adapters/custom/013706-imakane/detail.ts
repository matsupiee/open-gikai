...
import type { ImakanePdfRecord } from "./list";
 ...
 * 例: 「令和７年第２回今金町議会定例会　第１号」
 */
export function extractTitleFromText(text: string): string | null {
  const m = text.match(
    /(令和|平成|昭和)[０-９\d]+年第[０-９\d]+回今金町議会(定例会|臨時会)[^\n]*/,
  );
  if (m) return m[0].trim();

  // 委員会（通常表記 or スペース区切り表記）
  // PDF の「総 務 産 業 常 任 委 員 会」のように1文字ごとにスペースが入る場合も考慮し、
  // 各行を正規化してから検索する
...
async function extractTextFromPdf(buffer: ArrayBuffer): Promise<string | null> {
  try {
    ...
    console.warn("[imakane] unpdf extractText failed", e);
    return null;
  }
}
...
export async function fetchMeetingData(
  ...
  return {
    ...
    externalId: `imakane_${createHash("sha256").update(record.pdfUrl).digest("hex").slice(0, 16)}`,
    statements,
  };
}
