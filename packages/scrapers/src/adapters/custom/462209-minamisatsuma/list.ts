...
interface DvsCouncil {
  ...
  label: string; // 例: "令和7年第2回定例会"
  schedules: DvsSchedule[];
}

export interface MinamisatsumaRecord {
  /** 会議 ID */
  councilId: string;
  /** 日程 ID */
  scheduleId: string;
 ...
 */
export function parseCouncilList(data: unknown): MinamisatsumaRecord[] {
  if (!Array.isArray(data)) return [];

  const results: MinamisatsumaRecord[] = [];
  ...
  for (const council of data as DvsCouncil[]) {
    ...
    const sourceUrl = `${BASE_ORIGIN}/smart/tenant/minamisatsuma/WebView/rd/council_1.html`;
 ...
 */
export async function fetchDocumentList(year: number): Promise<MinamisatsumaRecord[]> {
  const url = `${BASE_ORIGIN}/dvsapi/councilrd/all?tenant_id=${TENANT_ID}&year=${year}`;
  const data = await fetchJsonp(url);
  if (!data) {
    console.warn(`[462209-minamisatsuma] No data returned for year=${year}`);
    return [];
  }
