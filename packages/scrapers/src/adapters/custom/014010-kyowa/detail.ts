...
export function parseSpeaker(text: string): {
  ...
  // 役職サフィックスにマッチする場合: "田中委員長" → name=田中, role=委員長
...
export async function fetchPdfStatements(
  ...
  try {
    ...
    console.warn(`[kyowa] PDF テキスト抽出失敗: ${pdfUrl}`, e);
    return null;
  }
...
export async function fetchMeetingData(
  ...
  return {
    ...
    externalId: `kyowa_${encodeURIComponent(doc.pdfUrl)}`,
    statements,
  };
}
