/**
 * 松崎町議会（静岡県） — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、○ マーカーで発言を分割して
 * ParsedStatement 配列を生成する。
 *
 * 発言フォーマット（議会会議録の一般的な形式）:
 *   ○議長（氏名君）  ただいまから…
 *   ○町長（氏名君）  お答えいたします。
 *   ○○番（氏名君）  質問いたします。
 */
 ...
 *   ○３番（野口君）        → role=議員, name=野口
 *   ○総務委員長（氏名君）  → role=委員長, name=氏名
 */
export function parseSpeaker(text: string): {
  ...
  // カッコパターンに合致しない場合（スペース区切り形式）
  // 例: ○渡辺議長 ただいまから本日の会議を開きます。
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[223051-matsuzaki] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  return {
    ...
    externalId: `matsuzaki_${params.docId}_${encodeURIComponent(params.pdfUrl)}`,
    statements,
  };
}
