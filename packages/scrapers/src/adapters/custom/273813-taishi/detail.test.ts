...
describe("parseSpeaker", () => {
  ...
  it("教育長パターンを解析する", () => {
    const result = parseSpeaker("○教育長（田中花子君）　お答えいたします。");
  ...
  it("副町長パターンを解析する", () => {
    const result = parseSpeaker("○副町長（高橋六郎君）　ご報告いたします。");
  ...
  it("委員長パターンを解析する", () => {
    const result = parseSpeaker(
      "○総務まちづくり常任委員会委員長（山口七郎君）　ご報告いたします。"
    );
    expect(result.speakerName).toBe("山口七郎");
    expect(result.speakerRole).toBe("委員長");
    expect(result.content).toBe("ご報告いたします。");
  });

  it("副委員長は委員長と誤マッチしない", () => {
    const result = parseSpeaker("○副委員長（伊藤八郎君）　進行します。");
    expect(result.speakerRole).toBe("副委員長");
    expect(result.speakerName).toBe("伊藤八郎");
  });

  it("名前に空白を含む場合は除去される", () => {
    const result = parseSpeaker("○町長（鈴木　一郎君）　答弁します。");
    expect(result.speakerName).toBe("鈴木一郎");
  });
