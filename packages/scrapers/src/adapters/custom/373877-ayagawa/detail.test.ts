...
describe("parseSpeaker", () => {
  ...
  it("町長（名前君）パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("綾川一郎");
  ...
  it("君/様なしの短縮名パターンを解析する（綾川町特有）", () => {
    const result = parseSpeaker(
      "○議長（河野）ただいまから会議を開きます。"
    );
    expect(result.speakerName).toBe("河野");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("ただいまから会議を開きます。");
  });

  it("〇（U+3007）マーカーの短縮名パターンを解析する", () => {
    const result = parseSpeaker(
      "\u3007教育長（香川）お答えいたします。"
    );
    expect(result.speakerName).toBe("香川");
    expect(result.speakerRole).toBe("教育長");
    expect(result.content).toBe("お答えいたします。");
  });
  ...
  it("委員長の短縮名パターンを解析する", () => {
    const result = parseSpeaker(
      "○議会運営委員長（福家利）ご報告いたします。"
    );
    expect(result.speakerName).toBe("福家利");
    expect(result.speakerRole).toBe("委員長");
    expect(result.content).toBe("ご報告いたします。");
  });

  it("番号議員の短縮名パターンを解析する", () => {
    const result = parseSpeaker(
      "○３番（田中）質問いたします。"
    );
...
`;
    ...
    expect(statements[2]!.speakerName).toBe("綾川一郎");
...
○３番（田中花子君）　質問があります。`;
  ...
  it("〇（U+3007）マーカーも正しく分割する", () => {
    const text = `○議長（河野）ただいまから会議を開きます。
\u3007教育長（香川）お答えいたします。`;

    const statements = parseStatements(text);

    expect(statements).toHaveLength(2);
    expect(statements[0]!.speakerName).toBe("河野");
    expect(statements[0]!.speakerRole).toBe("議長");
    expect(statements[0]!.kind).toBe("remark");

    expect(statements[1]!.speakerName).toBe("香川");
    expect(statements[1]!.speakerRole).toBe("教育長");
    expect(statements[1]!.kind).toBe("answer");
  });

  it("短縮名パターンで kind を正しく分類する", () => {
    const text = `○議長（河野）質問を許します。
○３番（田中）質問があります。
○町長（前田）お答えします。`;
