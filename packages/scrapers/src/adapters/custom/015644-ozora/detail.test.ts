...
describe("parseSpeaker", () => {
  it("議長（名前君）パターンを解析する", () => {
    const result = parseSpeaker(
      "○議長（中村豊君）　ただいまから会議を開きます。",
    );
    expect(result.speakerName).toBe("中村豊");
  ...
  it("町長（名前君）パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("山本勝");
...
`;
    ...
    expect(statements[0]!.speakerName).toBe("中村豊");
    ...
    expect(statements[2]!.speakerName).toBe("山本勝");
...
○３番（田中太郎君）　質問です。`;
  ...
  it("ト書き（登壇）はスキップする", () => {
    const text = `○議長（中村豊君）　ただいまから会議を開きます。
○（３番　田中太郎君登壇）
○３番（田中太郎君）　質問があります。`;
