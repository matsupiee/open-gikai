...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[465020-minamitane] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  // PDF URL のファイル名を externalId として利用
  const urlPath = new URL(meeting.pdfUrl).pathname;
  const fileName = urlPath.split("/").pop()?.replace(/\.pdf$/i, "") ?? null;
  const externalId = fileName ? `minamitane_${fileName}` : null;

  // heldOn: 年のみわかるため、年の1月1日をフォールバックとして使用
  // （実際の開催日は PDF テキストから取得できる場合がある）
