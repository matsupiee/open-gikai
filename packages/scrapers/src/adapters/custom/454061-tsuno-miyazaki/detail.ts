 ...
 *
 * 発言フォーマット例:
 *   ○議長（田中 浩二君） ただいまから会議を開きます。
 *   ○町長（河野 正和君） お答えいたします。
 *   ○○番（山田 勇君） 質問いたします。
 *   ○総務課長（黒木 誠二君） ご説明いたします。
 */

import { createHash } from "node:crypto";
import { extractText, getDocumentProxy } from "unpdf";
import type { MeetingData, ParsedStatement } from "../../../utils/types";
import type { TsunoMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（田中 浩二君） → role=議長, name=田中 浩二
 *   ○町長（河野 正和君） → role=町長, name=河野 正和
 *   ○○番（山田 勇君） → role=議員(番号付き), name=山田 勇
 *   ○総務課長（黒木 誠二君） → role=課長, name=黒木 誠二
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[454061-tsuno-miyazaki] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `tsuno_${idKey}` : null;
