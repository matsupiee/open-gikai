 ...
 * 志賀町議会 — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、「{名前}{役職}」マーカーで
 * 発言を分割して ParsedStatement 配列を生成する。
 *
 * 志賀町 PDF の形式:
 *   「福田晃悦議長 ただ今の出席議員は...」  -> name=福田晃悦, role=議長
 *   「稲岡健太郎町長 はい。」              -> name=稲岡健太郎, role=町長
 *   「南正紀議員 議長。」                  -> name=南正紀, role=議員
 */

import { createHash } from "node:crypto";
import { extractText, getDocumentProxy } from "unpdf";
import type { MeetingData, ParsedStatement } from "../../../utils/types";
import type { ShikaMeeting } from "./list";
 ...
 *
 * 入力例: "福田晃悦議長" -> { name: "福田晃悦", role: "議長" }
 * 入力例: "山下光雄町参事兼総務課長" -> { name: "山下光雄", role: "課長" }（最後の役職を取る）
 */
...
export async function fetchMeetingData(
  ...
  // PDF URL から externalId を抽出: /uploaded/attachment/{id}.pdf -> shika_{id}
  const idMatch = meeting.pdfUrl.match(/\/uploaded\/attachment\/(\d+)\.pdf$/i);
  const externalId = idMatch ? `shika_${idMatch[1]}` : null;
