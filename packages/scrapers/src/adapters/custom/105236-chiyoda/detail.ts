...
import type { ChiyodaMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（田口良雄君）　→ role=議長, name=田口良雄
 *   ○町長（大隅良一君）　→ role=町長, name=大隅良一
 *   ○３番（鈴木宏之君）　→ role=議員, name=鈴木宏之
 *   ○総務課長（小林正則君）→ role=課長, name=小林正則
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[105236-chiyoda] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `chiyoda_${idKey}` : null;
