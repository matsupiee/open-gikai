...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[105236-chiyoda] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[105236-chiyoda] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (err) {
    console.warn(
      `[105236-chiyoda] PDF 取得失敗: ${url}`,
      err instanceof Error ? err.message : err
    );
    return null;
  }
