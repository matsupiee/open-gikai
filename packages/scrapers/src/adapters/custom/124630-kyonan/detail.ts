 ...
 *
 * 発言フォーマット:
 *   ○議長（青木悦子）　それでは、ただいまから会議を開きます。
 *   ○町長（白石治和）　お答えいたします。
 *   ○１番（鈴木太郎）　質問いたします。
 *   ○税務住民課長（菊間寛之）　お答えいたします。
 */
...
export interface KyonanDetailParams {
  title: string;
  pdfUrl: string;
  meetingType: string;
  yearPageUrl: string;
 ...
 *
 * 対応パターン:
 *   ○議長（青木悦子）　→ role=議長, name=青木悦子
 *   ○町長（白石治和）　→ role=町長, name=白石治和
 *   ○１番（鈴木太郎）　→ role=議員, name=鈴木太郎
 *   ○税務住民課長（菊間寛之）→ role=課長, name=菊間寛之
 */
export function parseSpeaker(text: string): {
  ...
  // パターン: role（name）content（括弧内に「君」「様」「議員」がない鋸南スタイル）
...
/**
 * PDF テキストから開催日を抽出する。
 * パターン: 「令和７年５月８日　開会」「平成２４年３月５日」等（全角・半角数字両対応）
 */
...
export async function buildMeetingData(
  ...
  // externalId: PDFのURLからファイルIDを抽出
  const fileIdMatch = params.pdfUrl.match(/\/uploaded\/attachment\/(\d+)\.pdf/);
