...
describe("parseSpeaker", () => {
  ...
  it("番号付き議員の発言を抽出する", () => {
    const result = parseSpeaker("○3番（田中次郎君）　質問いたします。");
  ...
  it("副委員長を正しく識別する（委員長より長いため誤マッチしない）", () => {
    const result = parseSpeaker("○副委員長（山田花子君）　定足数を確認します。");
...
describe("classifyKind", () => {
  ...
  it("総務課長（endsWith 課長）は answer を返す", () => {
