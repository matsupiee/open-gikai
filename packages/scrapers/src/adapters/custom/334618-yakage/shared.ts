 ...
 * PDF ベースの議事録公開。1ページに全年度分の PDF リンクを掲載。
 * HTTP のみ対応（HTTPS 非対応）。
 */

export const BASE_ORIGIN = "http://www.town.yakage.okayama.jp";
export const LIST_PAGE_URL =
  "http://www.town.yakage.okayama.jp/gikaikaigiroku.html";
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(
        `[334618-yakage] fetchPage failed: ${url} status=${res.status}`
      );
      return null;
    }
    ...
    console.warn(
      `[334618-yakage] fetchPage error: ${url}`,
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(
        `[334618-yakage] fetchBinary failed: ${url} status=${res.status}`
      );
      return null;
    }
    ...
    console.warn(
      `[334618-yakage] fetchBinary error: ${url}`,
      e instanceof Error ? e.message : e
    );
    return null;
  }
