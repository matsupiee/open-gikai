/**
 * 七飯町議会 会議録 -- detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、発言者マーカーで発言を分割して
 * ParsedStatement 配列を生成する。
 *
 * 七飯町の発言フォーマット（PDF 抽出テキスト）:
...
import type { NanaeMeeting } from "./list";
...
export async function fetchMeetingData(
  ...
  const externalId = fileKey ? `nanae_${fileKey}` : null;
