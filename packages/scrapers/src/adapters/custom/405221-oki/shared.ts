...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[405221-oki] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(
      `[405221-oki] fetchPage error: ${url}`,
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[405221-oki] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(
      `[405221-oki] fetchBinary error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
...
export async function fetchJson(url: string): Promise<unknown> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[405221-oki] fetchJson failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.json();
  } catch (e) {
    console.warn(
      `[405221-oki] fetchJson error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
