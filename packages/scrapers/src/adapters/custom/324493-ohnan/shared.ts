 ...
 * 会議録トップページから年度別ページ URL を取得し、各年度ページから PDF リンクを収集する。
 */

export const BASE_ORIGIN = "https://www.town.ohnan.lg.jp";
export const TOP_PAGE_URL = `${BASE_ORIGIN}/www/genre/1000100000201/index.html`;
...
export function parseDateFromText(text: string): string | null {
  ...
  // 年号と月日の間に「第10回定例会（第5号）」のようなテキストが入ることがあるため
  // 年号の後に非貪欲ワイルドカードを使用する
  const fullMatch = normalized.match(/(令和|平成)(元|\d+)年[^年]*?(\d+)月(\d+)日/);
 ...
 * 会議種別を検出する。
 */
export function detectMeetingType(title: string): "extraordinary" | "plenary" {
