/**
 * 香美市議会 DiscussVision Smart — detail フェーズ
 *
 * schedule 単位で受け取った playlist アイテムを statements に変換する。
 *
 * 香美市は会議録テキスト（minute_text）が非公開のため、
...
function buildScheduleUrl(councilId: string, scheduleId: string): string {
  return `${BASE_ORIGIN}/smart/tenant/kami/WebView/rd/schedule.html?council_id=${councilId}&schedule_id=${scheduleId}`;
}
...
export function buildMeetingData(
  ...
  return {
    ...
    externalId: `kami_${params.councilId}_${params.scheduleId}`,
    statements,
  };
}
