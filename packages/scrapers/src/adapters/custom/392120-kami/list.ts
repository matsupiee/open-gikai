/**
 * 香美市議会 DiscussVision Smart — list フェーズ
 *
...
/** list フェーズで返すレコード */
export interface KamiListRecord {
  councilId: string;
  councilLabel: string;
  councilYear: string;
  scheduleId: string;
...
/**
 * API レスポンス（CouncilItem[]）から KamiListRecord[] に展開する。
 * テスト可能な純粋関数。
 */
export function expandCouncils(councils: CouncilItem[]): KamiListRecord[] {
  const records: KamiListRecord[] = [];
