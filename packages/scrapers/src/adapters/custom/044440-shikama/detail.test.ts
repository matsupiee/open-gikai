...
describe("parseSpeaker", () => {
  it("議長（氏名君）パターンを解析する", () => {
    const result = parseSpeaker("○議長（山田一郎君）　ただいまから本日の会議を開きます。");
  ...
  it("番号付き議員パターンを解析する", () => {
    const result = parseSpeaker("○１番（佐藤三郎君）　質問いたします。");
  ...
  it("副町長パターンを解析する", () => {
    const result = parseSpeaker("○副町長（田中四郎君）　ご説明いたします。");
  ...
  it("教育長パターンを解析する", () => {
    const result = parseSpeaker("○教育長（高橋五郎君）　お答えします。");
  ...
  it("議会事務局長パターンを解析する", () => {
    const result = parseSpeaker("○議会事務局長（伊藤六郎君）　ご報告いたします。");
  ...
  it("副議長パターンを解析する", () => {
    const result = parseSpeaker("○副議長（渡辺七郎君）　発言を許します。");
    expect(result.speakerName).toBe("渡辺七郎");
    expect(result.speakerRole).toBe("副議長");
    expect(result.content).toBe("発言を許します。");
  });

  it("名前に空白を含む場合は除去される", () => {
    const result = parseSpeaker("○町長（中村　八郎君）　答弁します。");
    expect(result.speakerName).toBe("中村八郎");
  });
...
`;
    ...
    expect(statements[0]!.speakerName).toBe("山田一郎");
...
○１番（佐藤三郎君）　質問です。`;
  ...
  it("ト書き（登壇）はスキップする", () => {
    const text = `○議長（山田一郎君）　ただいまから会議を開きます。
（１番　佐藤三郎君登壇）
○１番（佐藤三郎君）　質問があります。`;
...
describe("extractHeldOnFromPdfText", () => {
  it("令和の日付を抽出する（全角数字）", () => {
    const text = `令和６年１２月１５日（月曜日）

第４回色麻町議会定例会会議録`;
    expect(extractHeldOnFromPdfText(text)).toBe("2024-12-15");
  });
