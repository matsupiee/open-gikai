...
import type { ShikamaMeeting } from "./list";
 ...
 *   ○町長（山田太郎君）　お答えします → role=町長, name=山田太郎
 *   ○１番（佐藤一郎君）　質問します → role=議員, name=佐藤一郎
 *   ○副町長（鈴木花子君）　ご説明します → role=副町長, name=鈴木花子
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[044440-shikama] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = `shikama_${fileName.replace(".pdf", "")}`;
