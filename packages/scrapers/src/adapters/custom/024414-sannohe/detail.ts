...
import type { SannoheMeeting } from "./list";
 ...
 * 三戸町議会の PDF は冒頭に目次と議案件名・出席議員名簿等があり、
 * ○ マーカー付きのエントリが含まれるため誤パースの原因になる。
 *
 * 本文は「○議長」で始まるため、最初の本文中「○議長」以降を返す。
 * 目次内の「○議長」は直後に「……」が続くため区別できる。
 */
export function stripToc(text: string): string {
  ...
  while (true) {
    ...
    const blockEnd = nextMarker !== -1 ? nextMarker : Math.min(text.length, idx + 200);
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[024414-sannohe] PDF 取得失敗: ${pdfUrl}`,
 ...
 * e.g., "521kaigiroku.pdf" → "521kaigiroku"
 *       "519kessantokubetu.pdf" → "519kessantokubetu"
 */
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `sannohe_${idKey}` : null;
