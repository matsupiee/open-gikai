...
describe("stripToc", () => {
  it("目次と前文を除去して○議長から始まる本文のみ返す", () => {
    const text = `目 次 ○髙山一郎君（再質問）…………………………………２３ 答弁（町長 田中二郎君）…………………………………………２３ ○ 町長提出議案件名 ○応招議員 ○議長（川村浩昭君） これより本日の会議を開きます。`;
...
describe("extractExternalIdKey", () => {
  ...
  it("決算特別委員会のファイル名からキーを抽出する", () => {
    expect(
      extractExternalIdKey(
        "https://www.town.sannohe.aomori.jp/material/files/group/13/519kessantokubetu.pdf",
      ),
    ).toBe("519kessantokubetu");
  });

  it("臨時会のファイル名からキーを抽出する", () => {
    expect(
      extractExternalIdKey(
        "https://www.town.sannohe.aomori.jp/material/files/group/13/515rinjikaikaigiroku.pdf",
      ),
    ).toBe("515rinjikaikaigiroku");
  });

  it("マッチしないURLはnullを返す", () => {
    expect(extractExternalIdKey("https://example.com/")).toBeNull();
  });
});
