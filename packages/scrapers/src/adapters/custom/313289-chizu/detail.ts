...
import type { ChizuMeeting } from "./list";
...
export function parseSpeaker(text: string): {
  ...
  // パターン2: {role}（{name}）content (e.g., 議長（谷口雅人）)
...
export async function fetchMeetingData(
  ...
  const externalId = filename ? `chizu_${filename}` : null;
