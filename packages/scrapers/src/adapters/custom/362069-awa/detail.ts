...
export interface AwaDetailParams {
  title: string;
  heldOn: string;
  pdfUrl: string;
  meetingType: string;
 ...
 *   ○市長（町田寿人君）       → role=市長, name=町田寿人
 *   ○総務常任委員長（坂東重夫君） → role=委員長, name=坂東重夫
 *   ○理事（坂東孝一君）       → role=理事, name=坂東孝一
 *   ○市民部長（稲井誠司君）    → role=部長, name=稲井誠司
...
export async function buildMeetingData(
  ...
  return {
    ...
    externalId: `awa_${params.pageId}_${params.heldOn}`,
    statements,
  };
}
