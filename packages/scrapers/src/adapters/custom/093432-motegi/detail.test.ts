...
describe("parseSpeaker", () => {
  it("議長（名前君）パターンを解析する", () => {
    const result = parseSpeaker(
      "○議長（古田土志郎君）　ただいまから本日の会議を開きます。"
    );
    expect(result.speakerName).toBe("古田土志郎");
  ...
  it("町長（名前君）パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("古口達也");
  ...
  it("番号付き議員パターンを解析する（全角数字）", () => {
    ...
    expect(result.speakerName).toBe("山口文明");
...
`;
    ...
    expect(statements[0]!.speakerName).toBe("古田土志郎");
    expect(statements[0]!.speakerRole).toBe("議長");
    expect(statements[0]!.content).toBe("ただいまから本日の会議を開きます。");

    expect(statements[1]!.kind).toBe("question");
    expect(statements[1]!.speakerName).toBe("山口文明");
    expect(statements[1]!.speakerRole).toBe("議員");

    expect(statements[2]!.kind).toBe("answer");
    expect(statements[2]!.speakerName).toBe("古口達也");
...
○６番（山口文明君）　質問です。`;
  ...
  it("ト書き（登壇）はスキップする", () => {
    const text = `○議長（古田土志郎君）　ただいまから会議を開きます。
〔議会運営委員長　磯　稲藏君登壇〕
○６番（山口文明君）　質問があります。`;

    const statements = parseStatements(text);
    // 〔〕形式のト書きは ○ で始まらないのでもともとスキップされる
  ...
  it("◎ で始まるセクション見出しも remark として含まれる", () => {
    const text = `◎開会の宣告
○議長（古田土志郎君）　発言します。`;

    const statements = parseStatements(text);
    // ◎ から始まるセクション見出しも ○ と同様にブロック分割対象
...
１．期　日　平成31年4月1日
`;
    expect(extractHeldOnFromPdfText(text)).toBe("2019-04-01");
  });
