/**
 * 塙町議会 DiscussVision Smart — list フェーズ
 *
...
/** list フェーズで返すレコード */
export interface HanawaListRecord {
  councilId: string;
  councilLabel: string;
  councilYear: string;
  scheduleId: string;
...
/**
 * API レスポンス（CouncilItem[]）から HanawaListRecord[] に展開する。
 * テスト可能な純粋関数。
 */
export function expandCouncils(councils: CouncilItem[]): HanawaListRecord[] {
  const records: HanawaListRecord[] = [];
