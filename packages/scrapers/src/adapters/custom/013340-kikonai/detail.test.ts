...
describe("parseSpeaker", () => {
  ...
  it("町長（氏名君）パターンを解析する", () => {
    const result = parseSpeaker("○町長（石﨑倫行君）");
    expect(result.speakerName).toBeNull();
...
describe("parseStatements", () => {
  ...
  it("町長の発言を answer として分類する", () => {
    const text = `○町長（石﨑倫行君） お答えいたします。人口減少対策については、積極的に取り組んでまいります。`;
  ...
  it("複数の発言者を正しく抽出する", () => {
    const text = `○議長（田村一義君） 提案理由の説明を求めます。○町長（石﨑倫行君） 議案第１号について説明します。○3番（田村一義君） 質問いたします。`;
  ...
  it("offset が正しく計算される", () => {
    const text = `○議長（田村一義君） ただいま。○町長（石﨑倫行君） お答えします。`;
  ...
  it("発言マーカーがない場合は空配列を返す", () => {
    const text = `令和６年第１回木古内町議会定例会会議録`;
  ...
  it("空の発言内容はスキップする", () => {
    const text = `○議長（田村一義君） ○町長（石﨑倫行君） お答えします。`;
