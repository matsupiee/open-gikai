 ...
 * 自治体コード: 473081
 *
 * 会議録一覧ページ（単一ページ）から DOCX/PDF リンクを収集し、
 * 各ファイルをダウンロード・解析する。
 * - 全年度が単一ページに掲載（ページネーションなし）
 * - 令和6年〜: DOCX 形式
 * - 平成25年〜令和5年: PDF 形式
 */

export const BASE_ORIGIN = "https://www.town.motobu.okinawa.jp";

/** 会議録一覧ページ URL */
export const LIST_PAGE_URL = `${BASE_ORIGIN}/doc/2023103100062/`;
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[473081-motobu] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(
      `[473081-motobu] fetchPage error: ${url}`,
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[473081-motobu] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(
      `[473081-motobu] fetchBinary error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
