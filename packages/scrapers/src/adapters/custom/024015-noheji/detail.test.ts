...
describe("extractHeldOn", () => {
  it("令和の日付を抽出する", () => {
    const text = "令和6年3月5日　第1回野辺地町議会定例会会議録";
  ...
  it("日付の1桁部分に全角スペースがある場合も抽出する", () => {
    const text = "令和６年１２月　８日　第５回定例会";
    expect(extractHeldOn(text)).toBe("2024-12-08");
  });
...
describe("extractHeldOnFromLinkText", () => {
  it("定例会リンクテキストから開催日を抽出する（12月）", () => {
    const linkText = "本会議第２号（１２月　９日）【一般質問】";
    const sessionTitle = "令和３年第７回１２月定例会";
    expect(extractHeldOnFromLinkText(linkText, sessionTitle)).toBe("2021-12-09");
  });

  it("定例会リンクテキストから開催日を抽出する（3月）", () => {
    const linkText = "本会議第１号（３月　８日）【開会】";
    const sessionTitle = "令和４年第１回３月定例会";
    expect(extractHeldOnFromLinkText(linkText, sessionTitle)).toBe("2022-03-08");
  });

  it("日付パターンがない場合は null を返す", () => {
    const linkText = "本会議会議録目次";
    const sessionTitle = "令和６年第１回３月定例会";
    expect(extractHeldOnFromLinkText(linkText, sessionTitle)).toBeNull();
  });

  it("セッションタイトルが令和以外の場合は null を返す", () => {
    const linkText = "本会議第１号（３月　１日）【開会】";
    const sessionTitle = "無効なタイトル";
    expect(extractHeldOnFromLinkText(linkText, sessionTitle)).toBeNull();
  });
});
