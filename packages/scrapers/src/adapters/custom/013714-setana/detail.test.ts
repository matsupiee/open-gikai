...
describe("parseSpeaker", () => {
  ...
  it("副委員長（名前）形式をパースする", () => {
    const result = parseSpeaker("〇副委員長（伊藤四郎） 続きを行います。");
    expect(result.speakerName).toBe("伊藤四郎");
    expect(result.speakerRole).toBe("副委員長");
    expect(result.content).toBe("続きを行います。");
  });
...
describe("extractDateFromText", () => {
  ...
  it("元年の日付をパースする", () => {
    const text = "令和元年12月5日（木）";
    expect(extractDateFromText(text)).toBe("2019-12-05");
  });
...
describe("extractTitleFromText", () => {
  it("定例会タイトルを抽出する", () => {
    const text = "令和７年第２回せたな町議会定例会\n令和７年６月１２日（木）";
    const title = extractTitleFromText(text);
    expect(title).toBe("令和７年第２回せたな町議会定例会");
  });

  it("臨時会タイトルを抽出する", () => {
    const text = "令和５年第１回せたな町議会臨時会";
    const title = extractTitleFromText(text);
    expect(title).toBe("令和５年第１回せたな町議会臨時会");
  });

  it("常任委員会タイトルを抽出する", () => {
    const text = "総務産業常任委員会\n期日 令和６年１月２３日（火）";
    const title = extractTitleFromText(text);
    expect(title).not.toBeNull();
    expect(title).toContain("常任委員会");
  });
