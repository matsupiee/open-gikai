...
import type { TaiwaMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（佐々木義章君）　ただいまから → role=議長, name=佐々木義章
 *   ○町長（浅野元君）　お答えします → role=町長, name=浅野元
 *   ○１番（田中一郎君）　質問します → role=議員, name=田中一郎
 *   ○総務課長（鈴木次郎君）　ご説明します → role=課長, name=鈴木次郎
 */
...
export async function fetchMeetingData(
  ...
  const externalId = `taiwa_${fileName.replace(".pdf", "")}`;
