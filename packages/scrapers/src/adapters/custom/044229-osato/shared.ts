...
export const BASE_ORIGIN = "https://www.town.miyagi-osato.lg.jp";

/** 会議録一覧ページのパス */
export const LIST_PATH = "/site/oosato-gikai/kaigiroku.html";
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    console.warn(`[044229-osato] fetchPage failed: ${url}`, e);
    return null;
  }
}
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    console.warn(`[044229-osato] fetchBinary failed: ${url}`, e);
    return null;
  }
}
...
/**
 * 和暦の年テキストから西暦年を返す。
 * 例: "令和7年" → 2025, "令和元年" → 2019, "平成28年" → 2016
 * 解析できない場合は null を返す。
 */
