/**
 * 川辺町議会（岐阜県） — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、発言者パターンで分割して
 * ParsedStatement 配列を生成する。
 *
 * PDF テキストの特徴:
 * - スペース区切りで文字が分散することがある
 * - 発言者は ○ マーカーなし、役職（氏名君）パターン
 * - 議長、町長、副町長、課長等が発言者として登場
 */
...
export function parseDateFromText(text: string): string | null {
  ...
  const era = match[1]!;
  const rawEraYear = match[2]!;
  const eraYear =
    rawEraYear === "元" ? 1 : Number(toHalfWidth(rawEraYear));
...
export async function fetchMeetingData(
  ...
  const heldOn = parseDateFromText(normalizedText);
  ...
  const externalId = filenameMatch
    ? `kawabe_${filenameMatch[1]}`
    : null;

  return {
    municipalityId,
    title: meeting.linkText,
    meetingType: detectMeetingType(meeting.linkText),
    heldOn,
