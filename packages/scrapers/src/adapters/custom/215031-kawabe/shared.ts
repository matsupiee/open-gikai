...
export const BASE_ORIGIN = "https://www.kawabe-gifu.jp";
export const INDEX_URL = "https://www.kawabe-gifu.jp/?page_id=48191";
...
export function toJapaneseEra(year: number): string[] {
  ...
  if (year >= 2020) {
    ...
    results.push(`令和${eraYear}年`);
    // 全角数字バリアント
    const fullWidthYear = String(eraYear)
      .split("")
      .map((c) => String.fromCharCode(c.charCodeAt(0) + 0xfee0))
      .join("");
    results.push(`令和${fullWidthYear}年`);
  } else if (year === 2019) {
    results.push("令和元年");
    results.push("令和１年");
    results.push("平成31年");
