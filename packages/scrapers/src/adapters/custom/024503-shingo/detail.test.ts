...
describe("extractHeldOn", () => {
  it("令和の日付を抽出する（空白あり・半角）", () => {
    const text =
      "令和 6 年 3 月 1 日（木曜日）\n議事日程第 1 号";
    expect(extractHeldOn(text)).toBe("2024-03-01");
  });

  it("令和の日付を抽出する（空白なし・半角）", () => {
    expect(extractHeldOn("令和7年3月5日")).toBe("2025-03-05");
  });

  it("令和の日付を抽出する（全角数字・スペース混在）", () => {
    // 実際の新郷村PDFのパターン: "令 和６年 ３月 １ 日"
    expect(extractHeldOn("令 和６年 ３月 １ 日 開会")).toBe("2024-03-01");
  });

  it("全角数字のみの日付を抽出する", () => {
    expect(extractHeldOn("令和６年３月１日")).toBe("2024-03-01");
  });
