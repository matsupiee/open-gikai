...
import type { ShingoMeeting } from "./list";
...
/**
 * PDF テキストから開催日を抽出する。
 * PDF 冒頭部分に「令 和６年 ３月 １ 日」のようなパターンが含まれる。
 * 全角数字・空白が混在するため柔軟にマッチさせる。
 *
 * 対応パターン:
 *   - 「令和6年3月1日」（半角）
 *   - 「令和６年３月１日」（全角）
 *   - 「令 和６年 ３月 １ 日」（スペース混在・PDFから抽出された場合）
 */
export function extractHeldOn(text: string): string | null {
  // 全角数字を半角に変換
  const normalized = toHalfWidth(text);

  // 「令」と「和」の間にスペースが混入する場合も考慮した柔軟なパターン
  const match = normalized.match(
    /(令\s*和|平\s*成)\s*(元|\d+)\s*年\s*(\d+)\s*月\s*(\d+)\s*日/
  );
  if (!match) return null;

  const [, eraRaw, eraYearStr, monthStr, dayStr] = match;
  const era = eraRaw!.replace(/\s/g, ""); // スペースを除去して正規化
