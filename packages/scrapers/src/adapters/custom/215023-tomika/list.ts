 ...
 *
 * API エンドポイント:
 *   GET /dvsapi/councilrd/all?tenant_id=93&year={year}
 *
...
interface DvsCouncil {
  ...
  label: string; // 例: "令和7年12月定例会"
  schedules: DvsSchedule[];
}

export interface TomikaRecord {
  /** 会議 ID */
  councilId: string;
  /** 日程 ID */
  scheduleId: string;
 ...
 */
export function parseCouncilList(data: unknown): TomikaRecord[] {
  if (!Array.isArray(data)) return [];

  const results: TomikaRecord[] = [];
  ...
  for (const council of data as DvsCouncil[]) {
    ...
    const sourceUrl = `${BASE_ORIGIN}/smart/tenant/tomika/WebView/rd/council_1.html`;
 ...
 */
export async function fetchDocumentList(year: number): Promise<TomikaRecord[]> {
  const url = `${BASE_ORIGIN}/dvsapi/councilrd/all?tenant_id=${TENANT_ID}&year=${year}`;
  const data = await fetchJson(url);
  if (!data) {
    console.warn(`[215023-tomika] No data returned for year=${year}`);
    return [];
  }
