...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[215023-tomika] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(
      `[215023-tomika] fetchPage error: ${url}`,
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[215023-tomika] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(
      `[215023-tomika] fetchBinary error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
...
export async function fetchJson(url: string): Promise<unknown> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[215023-tomika] fetchJson failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.json();
  } catch (e) {
    console.warn(
      `[215023-tomika] fetchJson error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
