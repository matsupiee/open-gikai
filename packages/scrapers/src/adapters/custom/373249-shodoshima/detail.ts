...
import type { ShodoshimaMeeting } from "./list";
 ...
 *   ○町長（小豆島一郎君）　→ role=町長, name=小豆島一郎
 *   ○３番（田中花子君）　→ role=議員, name=田中花子
 *   ○議長（山田）　→ role=議長, name=山田  (君/様なし短縮名)
 */
export function parseSpeaker(text: string): {
  ...
  // パターン2: role（name）content  (君/様なしの短縮名パターン)
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[373249-shodoshima] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const pdfPath = new URL(meeting.pdfUrl).pathname;
  const idKey = extractExternalIdKey(pdfPath);
  const externalId = idKey ? `shodoshima_${idKey}` : null;
