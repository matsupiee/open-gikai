...
describe("parseSpeaker", () => {
  ...
  it("町長（名前君）パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("小豆島一郎");
  ...
  it("〇（U+3007）マーカーの短縮名パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("中田");
    expect(result.speakerRole).toBe("教育長");
    expect(result.content).toBe("お答えいたします。");
  });
  ...
  it("委員長の短縮名パターンを解析する", () => {
    const result = parseSpeaker(
      "○議会運営委員長（木村利）ご報告いたします。"
    );
    expect(result.speakerName).toBe("木村利");
...
`;
    ...
    expect(statements[2]!.speakerName).toBe("小豆島一郎");
...
○３番（田中花子君）　質問があります。`;
  ...
  it("〇（U+3007）マーカーも正しく分割する", () => {
    const text = `○議長（山田）ただいまから会議を開きます。
\u3007教育長（中田）お答えいたします。`;
