...
describe("parseSpeaker", () => {
  ...
  it("副議長パターンを抽出する", () => {
    const result = parseSpeaker("○副議長（田中花子君）　休憩いたします。");
  ...
  it("番号付き議員パターンを抽出する", () => {
    const result = parseSpeaker("○5番（佐藤次郎君）　質問いたします。");
  ...
  it("総務部長パターンから部長を抽出する", () => {
    const result = parseSpeaker("○総務部長（高橋三郎君）　ご説明いたします。");
  ...
  it("教育長パターンを抽出する", () => {
    const result = parseSpeaker("○教育長（伊藤四郎君）　お答えします。");
...
describe("parseStatements", () => {
  it("○マーカーで分割して ParsedStatement 配列を生成する", () => {
    ...
    expect(result[1]!.speakerName).toBe("佐藤次郎");
