...
describe("parseSpeaker", () => {
  ...
  it("番号議員パターンを解析する", () => {
    const result = parseSpeaker("○５番（鈴木一郎君）　質問いたします。");
  ...
  it("半角番号の議員パターンを解析する", () => {
    const result = parseSpeaker("○5番（鈴木一郎君）　質問いたします。");
    expect(result.speakerName).toBe("鈴木一郎");
    expect(result.speakerRole).toBe("議員");
    expect(result.content).toBe("質問いたします。");
  });
  ...
  it("副町長パターンを解析する", () => {
    const result = parseSpeaker("○副町長（田中次郎君）　ご報告します。");
  ...
  it("教育長パターンを解析する", () => {
    const result = parseSpeaker("○教育長（山本花子君）　お答えいたします。");
  ...
  it("課長パターンを解析する", () => {
    const result = parseSpeaker("○総務課長（加藤三郎君）　ご説明いたします。");
    expect(result.speakerName).toBe("加藤三郎");
    expect(result.speakerRole).toBe("課長");
    expect(result.content).toBe("ご説明いたします。");
  });

  it("副委員長パターンを解析する", () => {
    const result = parseSpeaker("○副委員長（中村四郎君）　ご説明いたします。");
