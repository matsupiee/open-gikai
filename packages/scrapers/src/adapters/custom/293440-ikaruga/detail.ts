...
import type { IkarugaMeeting } from "./list";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[293440-ikaruga] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const fileName = urlPath.split("/").pop()?.replace(".pdf", "") ?? null;
  const externalId = fileName ? `ikaruga_${fileName}` : null;
