...
describe("parseSpeaker", () => {
  ...
  it("教育長パターンを解析する", () => {
    const result = parseSpeaker("○教育長（高橋一郎君）　お答えいたします。");
    expect(result.speakerName).toBe("高橋一郎");
    expect(result.speakerRole).toBe("教育長");
    expect(result.content).toBe("お答えいたします。");
  });

  it("副町長パターンを解析する", () => {
    const result = parseSpeaker("○副町長（鈴木次郎君）　ご説明いたします。");
  ...
  it("部長パターンを解析する", () => {
    const result = parseSpeaker("○総務部長（伊藤三郎君）　ご報告いたします。");
  ...
  it("委員長パターンを解析する", () => {
    const result = parseSpeaker("○委員長（渡辺四郎君）　ただいまから委員会を開会します。");
  ...
  it("名前に空白を含む場合は除去される", () => {
    const result = parseSpeaker("○町長（山田　次郎君）　答弁します。");
    expect(result.speakerName).toBe("山田次郎");
  });
...
`;
  ...
  it("各 statement に contentHash が付与される", () => {
    const statements = parseStatements("○議長（田中太郎君）　テスト発言。");
