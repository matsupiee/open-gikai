...
import type { UchinadaMeeting } from "./list";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[173657-uchinada] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  // PDF URL から externalId を抽出: /uploaded/attachment/{id}.pdf -> uchinada_{id}
  const idMatch = meeting.pdfUrl.match(/\/uploaded\/attachment\/(\d+)\.pdf$/i);
  const externalId = idMatch ? `uchinada_${idMatch[1]}` : null;

  // タイトルは会議録ページタイトル + 内容ラベルを組み合わせる
  const title = meeting.contentLabel
    ? `${meeting.pageTitle} ${meeting.contentLabel}`
    : meeting.pageTitle;
