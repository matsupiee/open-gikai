...
describe("parseMeetingDateFromText", () => {
  it("令和年月日を YYYY-MM-DD にパースする", () => {
    const text = `令和6年第1回定例会会議録
令和6年3月7日（月曜日）　午前10時00分開議`;

    expect(parseMeetingDateFromText(text)).toBe("2024-03-07");
  });

  it("平成年月日を YYYY-MM-DD にパースする", () => {
    const text = `平成30年第1回定例会
平成30年3月5日（月曜日）　午前10時開議`;

    expect(parseMeetingDateFromText(text)).toBe("2018-03-05");
  });

  it("令和元年をパースする", () => {
    const text = `令和元年第1回定例会
令和元年6月10日（月曜日）`;
...
describe("extractExternalIdKey", () => {
  it("/fs/ パスからキーを抽出する", () => {
    const key = extractExternalIdKey(
      "https://www.city.sukumo.kochi.jp/fs/7/0/0/5/0/_/_____6__1____.pdf"
    );
    expect(key).toBe("7_0_0_5_0________6__1____");
  });

  it("シンプルな /fs/ パスからキーを抽出する", () => {
    const key = extractExternalIdKey(
      "https://www.city.sukumo.kochi.jp/fs/old/h1706.pdf"
    );
    expect(key).toBe("old_h1706");
  });

  it("/fs/ を含まない URL は null を返す", () => {
    const key = extractExternalIdKey(
      "https://www.city.sukumo.kochi.jp/docs/file.pdf"
    );
    expect(key).toBeNull();
  });
});
