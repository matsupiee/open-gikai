...
export interface HikawaDetailParams {
  title: string;
...
export async function buildMeetingData(
  ...
  // 最初の PDF テキストから開催日を抽出
  const heldOn = extractHeldOn(allTexts[0]!);
  ...
  return {
    ...
    externalId: `hikawa_${params.title}`,
    statements,
  };
}
