 ...
 *   ○地域政策課長（壱岐 和寿君） ご説明いたします。
 */

import { createHash } from "node:crypto";
import { extractText, getDocumentProxy } from "unpdf";
import type { MeetingData, ParsedStatement } from "../../../utils/types";
import type { KijoMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（甲斐 政治） → role=議長, name=甲斐 政治
 *   ○町長（半渡 英俊君） → role=町長, name=半渡 英俊
 *   ○議員（５番 桑原 勝広君） → role=議員, name=桑原 勝広
 *   ○地域政策課長（壱岐 和寿君） → role=課長, name=壱岐 和寿
 */
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `kijo_${idKey}` : null;
