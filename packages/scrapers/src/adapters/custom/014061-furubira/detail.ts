...
export interface FurubiraDetailParams {
  title: string;
  pdfUrl: string;
  meetingType: string;
  headingYear: number;
 ...
 *
 * 対応パターン:
 *   〇議長（堀 清君）　→ role=議長, name=堀清
 *   〇町長（成田昭彦君）　→ role=町長, name=成田昭彦
 *   〇５番（真貝政昭君）　→ role=議員, name=真貝政昭
 *   〇総務課長（細川正善君）→ role=課長, name=細川正善
 *   〇議会事務局長（白岩 豊君）→ role=事務局長, name=白岩豊
 *   〇会計管理者（関口央昌君）→ role=管理者, name=関口央昌
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[014061-furubira] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  return {
    ...
    externalId: `furubira_${heldOn}_${params.title}`,
    statements,
  };
}
