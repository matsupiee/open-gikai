...
export interface ShiroishiDetailParams {
  title: string;
...
/**
 * PDF テキストから開催日を抽出する。
 * パターン: 「令和６年１２月１３日」「平成２９年３月６日」等（全角・半角数字両対応）
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[414255-shiroishi] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  return {
    ...
    externalId: `shiroishi_${params.title}`,
    statements,
  };
}
