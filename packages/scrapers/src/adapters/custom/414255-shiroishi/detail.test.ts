...
describe("extractHeldOn", () => {
  it("全角数字の和暦日付を抽出する", () => {
    const text = "令和７年３月６日 午前１０時００分開議";
    expect(extractHeldOn(text)).toBe("2025-03-06");
  });

  it("半角数字の和暦日付を抽出する", () => {
    const text = "令和6年9月10日 午前10時00分開議";
    expect(extractHeldOn(text)).toBe("2024-09-10");
  });

  it("平成の日付を抽出する", () => {
    const text = "平成２９年３月６日 午前１０時００分開議";
    expect(extractHeldOn(text)).toBe("2017-03-06");
  });

  it("令和元年に対応する", () => {
    const text = "令和元年９月１０日 午前１０時開議";
