...
import type { SeiyoMeeting } from "./list";
 ...
 *   ○市長（山田一郎君）　→ role=市長, name=山田一郎
 *   ○３番（佐藤次郎君）  → role=議員, name=佐藤次郎
 *   ○総務課長（鈴木一君）→ role=課長, name=鈴木一
 */
...
export function parseStatements(text: string): ParsedStatement[] {
  ...
  for (const block of blocks) {
    ...
    // ト書き（登壇等）をスキップ: 発言内容部分が登壇・退席などのみのブロック
...
export async function fetchMeetingData(
  ...
  const externalId = fileName ? `seiyo_${fileName}` : null;
