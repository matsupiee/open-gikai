...
export interface TabuseDetailParams {
  title: string;
  year: number;
  pdfUrl: string;
  meetingType: string;
 ...
 *   ○総務委員長（鈴木次郎君） → role=委員長, name=鈴木次郎
...
export async function buildMeetingData(
  ...
  // externalId: 詳細ページID + PDFのファイルID から生成
  const detailPageId = params.detailUrl.match(/\/(\d+)\.html$/)?.[1] ?? "unknown";
  const pdfFileId = params.pdfUrl.match(/\/(\d+)\.pdf$/)?.[1] ?? "unknown";
  const externalId = `tabuse_${detailPageId}_${pdfFileId}`;

  // 開催日は PDF タイトルから月を抽出して推定する
  // 例: "令和6年第7回(12月)定例会会議録" → 12月
  const monthMatch = params.title.match(/\((\d+)月\)/);
  const month = monthMatch ? monthMatch[1]!.padStart(2, "0") : "01";
  const heldOn = `${params.year}-${month}-01`;
