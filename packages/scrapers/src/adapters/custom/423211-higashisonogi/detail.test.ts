...
describe("parseSpeaker", () => {
  it("議長パターンを抽出する", () => {
    const result = parseSpeaker(
      "○議長（田添 洋君） ただいまから会議を開きます。",
    );
    expect(result.speakerName).toBe("田添洋");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("ただいまから会議を開きます。");
  });

  it("町長パターンを抽出する", () => {
    const result = parseSpeaker("○町長（岡田伊一郎君） お答えいたします。");
    expect(result.speakerName).toBe("岡田伊一郎");
  ...
  it("副町長パターンを抽出する", () => {
    const result = parseSpeaker("○副町長（鈴木一郎君） お答えします。");
  ...
  it("教育長パターンを抽出する", () => {
    const result = parseSpeaker("○教育長（高橋花子君） お答えいたします。");
    expect(result.speakerName).toBe("高橋花子");
  ...
  it("名前にスペースが含まれる場合スペースを除去する", () => {
    const result = parseSpeaker("○議長（田添　洋君） 開会します。");
    expect(result.speakerName).toBe("田添洋");
    expect(result.speakerRole).toBe("議長");
  });

  it("〇 (U+3007) マーカーにも対応する", () => {
    const result = parseSpeaker("〇議長（田添洋君） 開会します。");
    expect(result.speakerName).toBe("田添洋");
    expect(result.speakerRole).toBe("議長");
  ...
  it("◯ (U+25EF) マーカーにも対応する", () => {
    const result = parseSpeaker("◯議長（田添洋君） 開会します。");
    expect(result.speakerName).toBe("田添洋");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("開会します。");
  });
...
describe("parseStatements", () => {
  it("○マーカーで分割して ParsedStatement 配列を生成する", () => {
    ...
    expect(result[0]!.speakerName).toBe("田添洋");
    ...
    expect(result[2]!.speakerName).toBe("岡田伊一郎");
