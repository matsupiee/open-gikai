...
export interface HigashisonogiDetailParams {
  title: string;
  pdfUrl: string;
  meetingType: string;
  headingYear: number;
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[423211-higashisonogi] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  params: HigashisonogiDetailParams,
  ...
  return {
    ...
    externalId: `higashisonogi_${params.heldOn}_${params.title}`,
    statements,
  };
}
