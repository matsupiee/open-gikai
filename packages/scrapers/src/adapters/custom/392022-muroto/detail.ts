...
import type { MurotoMeeting } from "./list";
...
export async function fetchMeetingData(
  ...
  // タイトルを PDF テキストから取得できれば優先、なければリンクテキストを使用
  const title = parseMeetingTitleFromText(text) ?? meeting.title;

  const idKey = extractExternalIdKey(meeting.pdfUrl);
  const externalId = idKey ? `muroto_${idKey}` : null;
