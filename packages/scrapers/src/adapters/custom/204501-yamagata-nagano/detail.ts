...
import { LIST_URL, fetchBinary } from "./shared";
...
/**
 * PDF テキストから開催日を抽出する。
 * パターン: 「令和６年１２月１３日」「平成３１年３月１日」等（全角・半角数字両対応）
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[204501-yamagata-nagano] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  params: { title: string; pdfUrl: string; meetingType: string },
  ...
  return {
    ...
    heldOn,
    sourceUrl: LIST_URL,
    externalId: `yamagata-nagano_${params.pdfUrl}`,
    statements,
  };
}
