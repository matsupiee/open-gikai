...
describe("parseSpeaker", () => {
  ...
  it("副議長を正しくパースする", () => {
    const result = parseSpeaker("○副議長（中村三郎君）　暫時休憩いたします。");
...
describe("parseStatements", () => {
  it("○ マーカーで発言を分割する", () => {
    const text = `○議長（山田一郎君）　ただいまから本日の会議を開きます。
○３番（田中太郎君）　質問いたします。
○町長（鈴木次郎君）　お答えいたします。`;
...
○３番（田中太郎君）　質問です。`;
  ...
  it("発言のない PDF テキストは空配列を返す", () => {
    const text = "上牧町議会 令和7年 第4回定例会 会議録";
    const statements = parseStatements(text);
