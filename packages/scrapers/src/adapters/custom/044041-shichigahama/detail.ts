 ...
 *   ○総務課長兼デジタル推進室長（氏名君）　ご説明いたします。
 */

import { createHash } from "node:crypto";
import { extractText, getDocumentProxy } from "unpdf";
import type { MeetingData, ParsedStatement } from "../../../utils/types";
import type { ShichigahamaMeeting } from "./list";
 ...
 *
 * 対応パターン:
 *   ○議長（安倍敏彦君）　ただいまから → role=議長, name=安倍敏彦
 *   ○副町長（平山良一君）　おはようございます → role=副町長, name=平山良一
 *   ○１番（田中一郎君）　質問します → role=議員, name=田中一郎
 *   ○総務課長兼デジタル推進室長（氏名君）　 → role=課長, name=氏名
 */
...
export function parseStatements(text: string): ParsedStatement[] {
  ...
  for (const block of blocks) {
    ...
    // ト書き（登壇等）をスキップ
    if (/^[○◯◎●]\s*[（(].+?(?:登壇|降壇|退席|退場|着席)[）)]$/.test(trimmed))
      continue;

    // 〔〕で囲まれた登壇・降壇表記をスキップ（行全体が〔〕の場合）
    if (/^\[.+?(?:登壇|降壇)\]$/.test(trimmed)) continue;
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[044041-shichigahama] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const fileName = decodeURIComponent(urlPath.split("/").pop() ?? urlPath);
  const externalId = `shichigahama_${fileName.replace(/\.pdf$/i, "")}`;
