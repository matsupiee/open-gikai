/**
 * 岐阜県大野町議会 — detail フェーズ
 *
 * 一般質問の PDF をダウンロードしてテキストを抽出し、
 * ○ マーカーで発言を分割して ParsedStatement 配列を生成する。
 *
 * 発言フォーマット（想定）:
 *   ○議長（山田太郎君）　それでは、一般質問を行います。
 *   ○○番（鈴木花子君）　質問いたします。
 *   ○町長（佐藤次郎君）　お答えいたします。
 */
...
import type { OnoGifuMeeting } from "./list";
import { fetchBinary, parseDateText } from "./shared";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[214035-ono-gifu] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  // heldOn: meeting 経由で渡された値を優先し、なければ PDF テキストから抽出
  const heldOn = meeting.heldOn ?? extractHeldOn(text);
  if (!heldOn) return null;

  // PDF URL のファイル名部分を externalId として利用
