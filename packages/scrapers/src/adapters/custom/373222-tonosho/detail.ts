...
import type { TonoshoMeeting } from "./list";
 ...
 *   ○町長（土庄一郎君）　→ role=町長, name=土庄一郎
 *   ○３番（田中花子君）　→ role=議員, name=田中花子
 *   ○議長（河野）　→ role=議長, name=河野
 */
 ...
 * PDF URL からテキストを取得する。
 */
async function fetchPdfText(pdfUrl: string, referer: string): Promise<string | null> {
  try {
    const buffer = await fetchBinary(pdfUrl, referer);
    ...
    console.warn(
      `[373222-tonosho] PDF 取得失敗: ${pdfUrl}`,
