...
export const BASE_ORIGIN = "https://www.town.rifu.miyagi.jp";

/** 会議録トップ（年度一覧）ページのパス */
export const TOP_PATH = "/gyosei/chosei/rifuchogikai/2/index.html";
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    console.warn(`[044067-rifu] fetchPage failed: ${url}`, e);
    return null;
  }
}
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    console.warn(`[044067-rifu] fetchBinary failed: ${url}`, e);
    return null;
  }
}
 ...
 * 例: "令和6年12月3日" → "2024-12-03"
 * 例: "令和元年6月1日" → "2019-06-01"
 * 解析できない場合は null を返す。
 */
