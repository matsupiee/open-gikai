...
import type { RifuMeeting } from "./list";
import { detectMeetingType, fetchBinary, parseWarekiDate, toHankaku } from "./shared";
 ...
 *
 * 対応パターン:
 *   ○議長（鈴木一郎君）　ただいまから → role=議長, name=鈴木一郎
 *   ○町長（佐藤太郎君）　お答えします → role=町長, name=佐藤太郎
 *   ○１番（田中花子君）　質問します → role=議員, name=田中花子
 *   ○議会事務局長（山田次郎君）　 → role=議会事務局長, name=山田次郎
 */
...
export function extractHeldOnFromPdfText(text: string): string | null {
  return parseWarekiDate(toHankaku(text));
}
...
export async function fetchMeetingData(
  ...
  const externalId = `rifu_${fileName.replace(".pdf", "")}`;
