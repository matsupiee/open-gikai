...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[023876-nakadomari] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  // heldOn: PDF テキストから抽出、失敗した場合は null
  ...
  const externalId = filename ? `nakadomari_${filename}` : null;
