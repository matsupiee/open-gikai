 ...
 * 本山町議会 — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、発言を分割して MeetingData を生成する。
 *
 * 発言フォーマット（PDF テキスト）:
...
import type { MotoyamaPdfEntry } from "./list";
...
/**
 * PDF ラベルテキストから開催日（YYYY-MM-DD）を抽出する。
 *
 * ラベル例: "12月3日 開会日", "9月10日 一般質問"
 * → 会期の年情報と組み合わせて YYYY-MM-DD を生成する
 *
 * @param label PDF リンクのラベルテキスト
 * @param heldOn 会期から取得した開始日（YYYY-MM-DD）。年の取得に使用
 */
export function parsePdfLabelDate(
  label: string,
  heldOn: string | null
): string | null {
  // "12月3日" パターンを抽出
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[393410-motoyama] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  entry: MotoyamaPdfEntry,
  municipalityCode: string
): Promise<MeetingData | null> {
  const text = await fetchPdfText(entry.pdfUrl);
  if (!text) return null;

  const statements = parseStatements(text);
  if (statements === null) return null;

  // 開催日: PDF ラベルから月日を取得し、会期の年と組み合わせる
  // 取得できない場合は null（heldOn=null の MeetingData は除外）
  let heldOn: string | null = parsePdfLabelDate(entry.label, entry.heldOn);
  if (!heldOn) {
    // フォールバック: 会期開始日をそのまま使う
    heldOn = entry.heldOn;
  }
  if (!heldOn) return null;

  // externalId: PDF ファイル名から生成
  const fileNameMatch = entry.pdfUrl.match(/\/([^/]+)\.pdf$/i);
  const fileName = fileNameMatch ? fileNameMatch[1]! : entry.pdfUrl;
  const externalId = `motoyama_${fileName}`;

  return {
    municipalityCode,
    title: entry.meetingTitle || entry.label,
    meetingType: detectMeetingType(entry.meetingTitle || entry.label),
    heldOn,
    sourceUrl: entry.yearPageUrl,
    externalId,
    statements,
  };
}
