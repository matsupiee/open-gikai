...
describe("extractHeldOn", () => {
  it("全角数字の和暦日付を抽出する", () => {
    const text = "令和６年３月１日　開会";
    expect(extractHeldOn(text)).toBe("2024-03-01");
  });
...
describe("parseSpeaker", () => {
  it("議長の発言を抽出する", () => {
    const result = parseSpeaker("○議長（山田太郎）　ただいまから会議を開きます。");
  ...
  it("番号付き議員の発言を抽出する（全角数字）", () => {
    const result = parseSpeaker("○１番（田中三郎）　質問いたします。");
  ...
  it("課長の発言を抽出する", () => {
    const result = parseSpeaker("○総務課長（佐藤四郎）　お答えいたします。");
  ...
  it("教育長の発言を抽出する", () => {
    const result = parseSpeaker("○教育長（中村五郎）　お答えいたします。");
  ...
  it("委員長の発言を抽出する", () => {
    const result = parseSpeaker("○議会運営委員会委員長（田中次郎）　報告いたします。");
    expect(result.speakerName).toBe("田中次郎");
  ...
  it("副委員長の発言を抽出する", () => {
    const result = parseSpeaker("○総務委員会副委員長（伊藤六郎）　報告します。");
  ...
  it("名前の間のスペースを除去する", () => {
    const result = parseSpeaker("○町長（鈴木　次郎）　お答えします。");
    expect(result.speakerName).toBe("鈴木次郎");
    expect(result.speakerRole).toBe("町長");
  });
...
describe("parseStatements", () => {
  it("○マーカーで発言を分割して ParsedStatement 配列を返す", () => {
    ...
    expect(result[1]!.speakerName).toBe("田中三郎");
