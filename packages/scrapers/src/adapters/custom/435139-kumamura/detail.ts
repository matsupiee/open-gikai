/**
 * 球磨村議会 会議録 — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、○ マーカーで発言を分割して
 * ParsedStatement 配列を生成する。
 *
 * 球磨村の会議録 PDF では発言者が ○ マーカーで区切られている想定。
 * フォーマット例:
 *   ○議長（○○○○君）　ただいまから会議を開きます。
 *   ○１番（○○○○君）　質問いたします。
 *   ○村長（○○○○君）　お答えいたします。
...
import type { KumamuraMeeting } from "./list";
import {
  buildDetailUrl,
  buildExternalId,
  detectMeetingType,
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[435139-kumamura] PDF 取得失敗: ${pdfUrl}`,
...
/**
 * 複数の PDF URL からテキストを結合して取得する。
 * 球磨村は1会議に複数の PDF（各日程）があるため、全て結合して処理する。
 */
async function fetchAllPdfText(pdfUrls: string[]): Promise<string | null> {
  const texts: string[] = [];
...
export async function fetchMeetingData(
  meeting: KumamuraMeeting,
  municipalityCode: string
): Promise<MeetingData | null> {
  const text = await fetchAllPdfText(meeting.pdfUrls);
  if (!text) return null;
  ...
  return {
    ...
    sourceUrl: buildDetailUrl(meeting.kijiId),
    externalId: buildExternalId(meeting.kijiId),
    statements,
  };
}
