...
describe("parseSpeaker", () => {
  it("議長パターンを抽出する", () => {
    const result = parseSpeaker("○議長（三浦恵美子君）　開議を宣告します。");

    expect(result.speakerName).toBe("三浦恵美子");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("開議を宣告します。");
  });

  it("町長パターンを抽出する", () => {
    const result = parseSpeaker("○町長（及川秀一郎君）　お答えいたします。");

    expect(result.speakerName).toBe("及川秀一郎");
    expect(result.speakerRole).toBe("町長");
    expect(result.content).toBe("お答えいたします。");
  });

  it("番号付き議員パターンを抽出する", () => {
    const result = parseSpeaker("○３番（木村洋昭君）　質問いたします。");

    expect(result.speakerName).toBe("木村洋昭");
    expect(result.speakerRole).toBe("議員");
    expect(result.content).toBe("質問いたします。");
  });

  it("半角番号付き議員パターンを抽出する", () => {
    const result = parseSpeaker("○3番（木村洋昭君）　質問いたします。");
  ...
  it("課長パターンを抽出する", () => {
    const result = parseSpeaker("○総務課長（田中太郎君）　お答えいたします。");
  ...
  it("副議長パターンを抽出する", () => {
    const result = parseSpeaker("○副議長（山田花子君）　議事を進めます。");
  ...
  it("名前の空白を除去する", () => {
    const result = parseSpeaker("○町長（及川　秀一郎君）　お答えします。");
  ...
  it("教育長パターンを抽出する", () => {
    const result = parseSpeaker("○教育長（佐藤一郎君）　お答えいたします。");
