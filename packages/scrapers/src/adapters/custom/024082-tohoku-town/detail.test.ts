...
import { normalizePdfText, parseJapaneseDate } from "./shared";

describe("normalizePdfText", () => {
  it("文字間スペースを除去して自然なテキストに復元する", () => {
    const spaced = "令 和 ６ 年 １ ２ 月 １ ０ 日 （ 火 曜 日 ）";
    const result = normalizePdfText(spaced);
    expect(result).toBe("令和6年12月10日（火曜日）");
  });

  it("〇（漢数字ゼロ）を ○（白丸）に変換する", () => {
    const text = "〇議長（山田太郎君）　発言します。";
    const result = normalizePdfText(text);
    expect(result).toContain("○議長");
  });

  it("全角数字を半角に変換する", () => {
    const text = "令和６年第４回定例会";
    const result = normalizePdfText(text);
    expect(result).toBe("令和6年第4回定例会");
  });
});

describe("parseJapaneseDate", () => {
  it("令和の年月日を西暦に変換する", () => {
    expect(parseJapaneseDate("令和6年12月10日")).toBe("2024-12-10");
    expect(parseJapaneseDate("令和7年3月1日")).toBe("2025-03-01");
  });
  ...
  it("平成の年月日を西暦に変換する", () => {
    expect(parseJapaneseDate("平成30年12月1日")).toBe("2018-12-01");
  });

  it("テキスト中に日付が含まれる場合も抽出する", () => {
    expect(parseJapaneseDate("令和6年12月10日（火曜日）午前10時00分開議")).toBe("2024-12-10");
  });

  it("日付が含まれない場合は null を返す", () => {
    expect(parseJapaneseDate("議事日程（第2号）")).toBeNull();
  });
});
