...
import type { NishiharaMeeting } from "./list";
import {
  buildDetailUrl,
  buildExternalId,
  detectMeetingType,
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[434329-nishihara] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const heldOn = meeting.heldOn ?? parseJapaneseDate(text);

  // heldOn が解析できない場合は null を返す（"1970-01-01" 禁止）
  if (!heldOn) return null;

  // アーカイブエントリは kiji003295 の sourceUrl を使用
  const sourceUrl = meeting.kijiId.startsWith("archive_")
    ? buildDetailUrl("003295")
    : buildDetailUrl(meeting.kijiId);

  const externalId = meeting.kijiId.startsWith("archive_")
    ? `nishihara_${meeting.pdfUrl.split("/").pop()?.replace(".pdf", "") ?? meeting.kijiId}`
    : buildExternalId(meeting.kijiId);
