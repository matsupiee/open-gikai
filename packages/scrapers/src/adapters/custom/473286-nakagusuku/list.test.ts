...
describe("fetchMeetingList", () => {
  ...
  it("council と schedule から ListRecord を生成する", async () => {
                  ...
                  council_type: [
                    {
                      ...
                      councils: [
                        { council_id: 45, name: "令和　６年　　９月　定例会（第３回）" },
                      ],
                    },
                  ],
                },
    ...
    // minutes/get_schedule for council_id=45
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () =>
        Promise.resolve({
    ...
    expect(params["tenantId"]).toBe(622);
    expect(params["tenantSlug"]).toBe("nakagusuku");
    expect(params["councilId"]).toBe(45);
    expect(params["scheduleId"]).toBe(301);
    expect(params["scheduleName"]).toBe("09月10日－01号");
    expect(params["viewYear"]).toBe("2024");
  });

  it("TENANT_ID=622 で councils/index API を呼び出す", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
                  ...
                  council_type: [
                    {
                      ...
                      councils: [
                        { council_id: 45, name: "令和　６年　　９月　定例会（第３回）" },
                        { council_id: 46, name: "令和　６年　１２月　定例会（第４回）" },
                      ],
                    },
                  ],
                },
