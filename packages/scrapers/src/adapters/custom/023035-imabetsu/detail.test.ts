...
describe("parseStatements", () => {
  it("テキストを段落に分割して ParsedStatement 配列を返す", () => {
    const text = `今別町議会だより第222号

令和7年2月3日発行

今回の定例会では、令和7年度の予算案について審議が行われました。

一般会計予算は総額20億円となり、前年度比3%増となっています。`;
...
今別町議会では令和7年度予算案について審議が行われ、様々な議案が提出されました。`;

    const statements = parseStatements(text);

    expect(statements).toHaveLength(1);
    expect(statements[0]!.content).toContain("今別町議会では");
  });

  it("contentHash が SHA-256 形式で生成される", () => {
    const text = `今別町議会だより第222号の内容がここに記載されています。`;
  ...
  it("offset が連続して計算される", () => {
    const text = `今別町議会では令和7年度予算案について審議が行われました。
...
一般会計予算は総額20億円となり、前年度比3%増となっています。`;
  ...
  it("複数の空行で正しく段落分割される", () => {
    const text = `第一段落の内容が記載されています。今別町の最新情報をお届けします。
...
第二段落の内容が記載されています。議会活動の詳細をお伝えします。`;
  ...
  it("各 statement の content が空でない", () => {
    const text = `今別町議会では令和7年度の一般会計補正予算が審議されました。
