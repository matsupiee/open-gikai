...
export interface TagaDetailParams {
  /** 会議名（例: 令和6年3月多賀町議会定例会） */
  sessionTitle: string;
  /** 会議録 PDF の絶対 URL */
  pdfUrl: string;
  /** PDF のリンクテキスト（例: 3月5日開会） */
  linkText: string;
  /** 会議種別 */
  meetingType: string;
...
export async function buildMeetingData(
  ...
  const externalId = `taga_${fileName}`;
