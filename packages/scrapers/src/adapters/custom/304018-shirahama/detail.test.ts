...
describe("cleanPdfText", () => {
  ...
  it("全角数字を半角に変換する", () => {
    const raw = "令 和 ６ 年 ９ 月 ３ 日";
    const cleaned = cleanPdfText(raw);
    expect(cleaned).toBe("令和6年9月3日");
  });
...
describe("parseJapaneseDate", () => {
  it("令和6年9月3日 → 2024-09-03", () => {
    expect(parseJapaneseDate("令和6年9月3日")).toBe("2024-09-03");
  });
...
describe("extractHeldOnFromText", () => {
  it("テキスト冒頭の日付を抽出する", () => {
    const text = `議会定例会会議録\n令和6年9月3日\n白浜町議会`;
    expect(extractHeldOnFromText(text)).toBe("2024-09-03");
  });
