/**
 * 白浜町議会（和歌山県） — detail フェーズ
 *
 * PDF をダウンロードし、unpdf でテキストを抽出して発言を構造化データに変換する。
 *
 * PDF のテキストは文字間にスペースが挟まれていることがあるため、除去して処理する。
 ...
 *
 * unpdf で抽出したテキストは各文字の間に空白が入ることがあるため、行ごとに除去する。
...
export function classifyKind(
  ...
  if (speakerRole === "委員長" || speakerRole === "副委員長") return "remark";
...
export function parseJapaneseDate(text: string): string | null {
  const reiwaMatch = text.match(/令和(元|\d+)年(\d+)月(\d+)日/);
  if (reiwaMatch) {
  ...
  const heiseiMatch = text.match(/平成(元|\d+)年(\d+)月(\d+)日/);
  if (heiseiMatch) {
 ...
 * PDF のテキストから開催日を抽出する。
 *
 * 通常、PDF の冒頭に「令和XX年X月X日」の形式で記載される。
 */
...
export async function fetchPdfStatements(
  ...
  try {
    ...
    console.warn("[304018-shirahama] PDF text extraction failed:", pdfUrl, e);
    return null;
  }

  const cleanedText = cleanPdfText(text);
  const heldOn = extractHeldOnFromText(cleanedText);
  const statements = parseStatements(cleanedText);
...
export async function fetchMeetingData(
  ...
  const externalId = `shirahama_${encodeURIComponent(params.pdfUrl.split("/").pop() ?? params.pdfUrl)}`;
