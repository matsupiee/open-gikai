/**
 * 美郷町議会（宮崎県）DiscussVision Smart — list フェーズ
 *
...
/** list フェーズで返すレコード */
export interface MisatoMiyazakiListRecord {
  councilId: string;
  councilLabel: string;
  councilYear: string;
  scheduleId: string;
...
/**
 * API レスポンス（CouncilItem[]）から MisatoMiyazakiListRecord[] に展開する。
 * テスト可能な純粋関数。
 */
export function expandCouncils(councils: CouncilItem[]): MisatoMiyazakiListRecord[] {
  const records: MisatoMiyazakiListRecord[] = [];
...
export async function fetchCouncilList(
  year: number,
): Promise<MisatoMiyazakiListRecord[]> {
