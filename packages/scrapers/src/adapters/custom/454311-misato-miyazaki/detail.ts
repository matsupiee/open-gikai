/**
 * 美郷町議会（宮崎県）DiscussVision Smart — detail フェーズ
 *
 * schedule 単位で受け取った playlist アイテムを statements に変換する。
 *
 * 美郷町は会議録テキスト（minute_text）が非公開のため、
...
import { BASE_ORIGIN, TENANT_SLUG, detectMeetingType, parseScheduleDate } from "./shared";
...
function buildScheduleUrl(councilId: string, scheduleId: string): string {
  return `${BASE_ORIGIN}/smart/tenant/${TENANT_SLUG}/WebView/rd/schedule.html?council_id=${councilId}&schedule_id=${scheduleId}`;
}
...
export function buildMeetingData(
  ...
  return {
    ...
    externalId: `misato_miyazaki_${params.councilId}_${params.scheduleId}`,
    statements,
  };
}
