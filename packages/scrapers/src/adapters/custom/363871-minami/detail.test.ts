...
describe("fetchMeetingData", () => {
  ...
  it("発言がある場合は MeetingData を返す", async () => {
          ...
          tenant_minutes: [
            {
              ...
              page_no: 1,
              hit_count: 0,
              body: "<pre>○議長（川尻竹藏）　おはようございます。ただいまから令和６年第３回美波町議会定例会を開会いたします。</pre>",
              minute_anchor_id: "",
              minute_type: "○議長",
              minute_type_code: 4,
              speech_type: null,
    ...
    expect(result!.municipalityCode).toBe("muni_363871");
    expect(result!.heldOn).toBe("2024-09-10");
    expect(result!.externalId).toBe("discussnet_ssp_426_41_201");
    expect(result!.sourceUrl).toContain("council_id=41");
    ...
    expect(result!.statements[0]!.speakerName).toBe("川尻竹藏");
  });
  ...
  it("minutes/get_minute API に正しいパラメータを渡す", async () => {
    ...
    expect(body.get("council_id")).toBe("41");
