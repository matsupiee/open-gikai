...
describe("fetchMeetingList", () => {
  ...
  it("council と schedule から ListRecord を生成する", async () => {
                  ...
                  council_type: [
                    {
                      ...
                      councils: [
                        { council_id: 41, name: "令和　６年　　９月　定例会（第３回）" },
                      ],
                    },
                  ],
                },
    ...
    // minutes/get_schedule for council_id=41
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: () =>
        Promise.resolve({
          council_schedules: [
            {
              ...
              hit_count: 0,
              postit_count: 0,
              member_list: "<pre>令和6年9月10日（火曜日）</pre>",
            },
          ],
        }),
    });
    ...
    expect(params["tenantId"]).toBe(426);
    expect(params["tenantSlug"]).toBe("minami");
    expect(params["councilId"]).toBe(41);
    expect(params["scheduleId"]).toBe(301);
    expect(params["scheduleName"]).toBe("09月10日－01号");
    expect(params["viewYear"]).toBe("2024");
  });

  it("TENANT_ID=426 で councils/index API を呼び出す", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
    ...
    expect(body.get("tenant_id")).toBe("426");
  });
                  ...
                  council_type: [
                    {
                      ...
                      councils: [
                        { council_id: 41, name: "令和　６年　　９月　定例会（第３回）" },
                        { council_id: 42, name: "令和　６年　１２月　定例会（第４回）" },
                      ],
                    },
                  ],
                },
