...
export interface KawaiDetailParams {
  title: string;
  heldOn: string | null;
  pdfUrl: string;
  meetingType: string;
 ...
 *
 * 対応パターン:
 *   ○議長（疋田俊文）　 → role=議長, name=疋田俊文
 *   ○町長（森川喜之）　 → role=町長, name=森川喜之
 *   ○３番（梅野美智代）　 → role=議員, name=梅野美智代
 *   ○総務部長（上村卓也）　 → role=部長, name=上村卓也
 */
...
export async function buildMeetingData(
  ...
  const externalId = `kawai_${fileName.replace(/\.pdf$/i, "")}`;
