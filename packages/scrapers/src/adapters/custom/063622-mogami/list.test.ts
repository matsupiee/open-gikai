...
describe("parseCouncilList", () => {
  it("会議一覧を正しくパースする", () => {
    ...
    expect(records[0]!.councilId).toBe("88");
    expect(records[0]!.scheduleId).toBe("1");
    expect(records[0]!.councilLabel).toBe("令和6年3月定例会");
    expect(records[0]!.scheduleLabel).toBe("03月07日　本会議");
    expect(records[0]!.heldOn).toBe("2024-03-15");
    expect(records[0]!.meetingType).toBe("plenary");

    expect(records[1]!.scheduleId).toBe("2");
    expect(records[1]!.scheduleLabel).toBe("03月08日　一般質問");
  ...
  it("sourceUrl が DiscussVision の mogami テナント URL を含む", () => {
    const data = [
      {
        council_id: "30",
        year: "2018-03-01",
    ...
    expect(records[0]!.sourceUrl).toContain("mogami");
  });
