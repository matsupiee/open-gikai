...
export async function fetchJsonp(url: string): Promise<unknown> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[063622-mogami] fetchJsonp failed: ${urlWithCb} status=${res.status}`);
      return null;
    }
    ...
    console.warn(
      `[063622-mogami] fetchJsonp error: ${urlWithCb}`,
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[063622-mogami] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(
      `[063622-mogami] fetchPage error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[063622-mogami] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(
      `[063622-mogami] fetchBinary error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
