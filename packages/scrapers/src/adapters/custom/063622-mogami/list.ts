 ...
 *
 * API エンドポイント:
 *   GET /dvsapi/councilrd/all?tenant_id=87&year={year}&callback={fn}
 *
 ...
 */
export function parseCouncilList(data: unknown): MogamiRecord[] {
  if (!Array.isArray(data)) return [];

  const results: MogamiRecord[] = [];
  ...
  for (const council of data as DvsCouncil[]) {
    ...
    const sourceUrl = `${BASE_ORIGIN}/smart/tenant/mogami/WebView/rd/council_1.html`;
 ...
 */
export async function fetchDocumentList(year: number): Promise<MogamiRecord[]> {
  const url = `${BASE_ORIGIN}/dvsapi/councilrd/all?tenant_id=${TENANT_ID}&year=${year}`;
  const data = await fetchJsonp(url);
  if (!data) {
    console.warn(`[063622-mogami] No data returned for year=${year}`);
    return [];
  }
