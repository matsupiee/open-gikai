...
export async function fetchJsonp(url: string): Promise<unknown> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[394017-nakatosa] fetchJsonp failed: ${urlWithCb} status=${res.status}`);
      return null;
    }
    ...
    console.warn(
      `[394017-nakatosa] fetchJsonp error: ${urlWithCb}`,
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[394017-nakatosa] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(
      `[394017-nakatosa] fetchPage error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[394017-nakatosa] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(
      `[394017-nakatosa] fetchBinary error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
