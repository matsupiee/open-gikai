...
export interface NakagawaNaganoDetailParams {
  pdfUrl: string;
  sessionTitle: string;
  heldOnLabel: string;
}
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[203866-nakagawa-nagano] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const heldOn = parseHeldOn(params.heldOnLabel);
  if (!heldOn) return null;

  // 添付 ID を externalId に使う
  const idMatch = params.pdfUrl.match(/\/uploaded\/attachment\/(\d+)\.pdf/);
