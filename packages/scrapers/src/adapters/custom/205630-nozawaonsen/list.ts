 ...
 *
 * API エンドポイント:
 *   GET /dvsapi/councilrd/all?tenant_id=602&year={year}&callback={fn}
 *
...
interface DvsCouncil {
  ...
  label: string; // 例: "令和6年3月定例会"
  schedules: DvsSchedule[];
}

export interface NozawaonsenRecord {
  /** 会議 ID */
  councilId: string;
  /** 日程 ID */
  scheduleId: string;
 ...
 */
export function parseCouncilList(data: unknown): NozawaonsenRecord[] {
  if (!Array.isArray(data)) return [];

  const results: NozawaonsenRecord[] = [];
  ...
  for (const council of data as DvsCouncil[]) {
    ...
    const sourceUrl = `${BASE_ORIGIN}/smart/tenant/nozawaonsen/WebView/rd/council_1.html`;
 ...
 */
export async function fetchDocumentList(year: number): Promise<NozawaonsenRecord[]> {
  const url = `${BASE_ORIGIN}/dvsapi/councilrd/all?tenant_id=${TENANT_ID}&year=${year}`;
  const data = await fetchJsonp(url);
  if (!data) {
    console.warn(`[205630-nozawaonsen] No data returned for year=${year}`);
    return [];
  }
