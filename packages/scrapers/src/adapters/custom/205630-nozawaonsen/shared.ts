...
export async function fetchJsonp(url: string): Promise<unknown> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[205630-nozawaonsen] fetchJsonp failed: ${urlWithCb} status=${res.status}`);
      return null;
    }
    ...
    console.warn(
      `[205630-nozawaonsen] fetchJsonp error: ${urlWithCb}`,
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[205630-nozawaonsen] fetchPage failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.text();
  } catch (e) {
    console.warn(
      `[205630-nozawaonsen] fetchPage error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[205630-nozawaonsen] fetchBinary failed: ${url} status=${res.status}`);
      return null;
    }
    return await res.arrayBuffer();
  } catch (e) {
    console.warn(
      `[205630-nozawaonsen] fetchBinary error: ${url}`,
      e instanceof Error ? e.message : e,
    );
    return null;
  }
