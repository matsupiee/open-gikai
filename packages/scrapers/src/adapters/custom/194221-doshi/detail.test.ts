...
describe("parseSpeaker", () => {
  it("議長（名前君）パターンを解析する", () => {
    const result = parseSpeaker(
      "○議長（杉山嘉英君） ただいまから本日の会議を開きます。",
    );
    expect(result.speakerName).toBe("杉山嘉英");
  ...
  it("村長（名前君）パターンを解析する", () => {
    const result = parseSpeaker(
      "○村長（長田富也君） 皆さん、おはようございます。",
    );
    expect(result.speakerName).toBe("長田富也");
  ...
  it("番号議員パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("仲田義一");
  ...
  it("課長パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("小林正樹");
...
`;
    ...
    expect(statements[0]!.speakerName).toBe("杉山嘉英");
    expect(statements[0]!.speakerRole).toBe("議長");

    expect(statements[1]!.kind).toBe("question");
    expect(statements[1]!.speakerName).toBe("仲田義一");
    expect(statements[1]!.speakerRole).toBe("議員");

    expect(statements[2]!.kind).toBe("answer");
    expect(statements[2]!.speakerName).toBe("長田富也");
...
○３番（仲田義一君） 質問です。`;
  ...
  it("ト書き（登壇）はスキップする", () => {
    const text = `○議長（杉山嘉英君） ただいまから会議を開きます。
○（３番　仲田義一君登壇）
○３番（仲田義一君） 質問があります。`;
  ...
  it("行政側の役職を正しく分類する", () => {
    const text = `○総務課長（小林正樹君） ご説明いたします。
○教育長（田中太郎君） お答えいたします。
○副村長（佐藤次郎君） 補足いたします。`;
    ...
    expect(statements[2]!.kind).toBe("answer");
    expect(statements[2]!.speakerRole).toBe("副村長");
  });
});
