...
describe("parseSpeaker", () => {
  it("議長パターンを抽出する", () => {
    ...
    expect(result.speakerName).toBe("西村光雄");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("ただいまから会議を開きます。");
  });

  it("町長パターンを抽出する", () => {
    const result = parseSpeaker("○町長（西村久之君） お答えいたします。");
    expect(result.speakerName).toBe("西村久之");
  ...
  it("番号付き議員パターン（全角）を抽出する", () => {
    const result = parseSpeaker("○１番（立石光助君） 質問いたします。");
    expect(result.speakerName).toBe("立石光助");
    expect(result.speakerRole).toBe("議員");
    expect(result.content).toBe("質問いたします。");
  });

  it("番号付き議員パターン（半角）を抽出する", () => {
    const result = parseSpeaker("○3番（森岡正雄君） 質問いたします。");
    expect(result.speakerName).toBe("森岡正雄");
    expect(result.speakerRole).toBe("議員");
    expect(result.content).toBe("質問いたします。");
  });
  ...
  it("名前にスペースが含まれる場合スペースを除去する", () => {
    const result = parseSpeaker("○議長（西村 光雄君） 開会します。");
    expect(result.speakerName).toBe("西村光雄");
    expect(result.speakerRole).toBe("議長");
  });

  it("〇 (U+3007) マーカーにも対応する", () => {
    const result = parseSpeaker("〇議長（西村光雄君） 開会します。");
    expect(result.speakerName).toBe("西村光雄");
    expect(result.speakerRole).toBe("議長");
  ...
  it("◯ (U+25EF) マーカーにも対応する", () => {
    const result = parseSpeaker("◯議長（西村光雄君） 開会します。");
    expect(result.speakerName).toBe("西村光雄");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("開会します。");
  });
...
describe("parseStatements", () => {
  it("○マーカーで分割して ParsedStatement 配列を生成する", () => {
    ...
    expect(result[0]!.speakerName).toBe("西村光雄");
    ...
    expect(result[1]!.speakerName).toBe("立石光助");
    expect(result[1]!.speakerRole).toBe("議員");

    expect(result[2]!.kind).toBe("answer");
    expect(result[2]!.speakerName).toBe("西村久之");
