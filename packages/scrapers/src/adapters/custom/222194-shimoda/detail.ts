...
import type { ShimodaMeeting } from "./list";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[222194-shimoda] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `shimoda_${idKey}` : null;
