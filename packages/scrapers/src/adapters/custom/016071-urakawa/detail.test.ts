...
describe("parseSpeaker", () => {
  ...
  it("副町長（名前）パターンを解析する", () => {
    const result = parseSpeaker("○副町長（佐々木花子）　御説明申し上げます。");
    expect(result.speakerName).toBe("佐々木花子");
    expect(result.speakerRole).toBe("副町長");
    expect(result.content).toBe("御説明申し上げます。");
  });

  it("教育長（名前）パターンを解析する", () => {
    const result = parseSpeaker("○教育長（高橋三郎）　お答えいたします。");
  ...
  it("番号議員（半角）パターンを解析する", () => {
    const result = parseSpeaker("○6番（田中花子）　質問いたします。");
  ...
  it("番号議員（全角）パターンを解析する", () => {
    const result = parseSpeaker("○６番（伊藤五郎）　発言します。");
  ...
  it("委員長パターンを解析する", () => {
    const result = parseSpeaker("○委員長（山本六郎）　ただいまから委員会を開会します。");
    expect(result.speakerName).toBe("山本六郎");
