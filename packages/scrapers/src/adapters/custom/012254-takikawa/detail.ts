...
import type { TakikawaMeeting } from "./list";
import {
  buildPageUrl,
  buildExternalId,
  detectMeetingType,
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[012254-takikawa] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const sourceUrl = buildPageUrl(meeting.pageId);
  const externalId = buildExternalId(meeting.pageId, meeting.attachmentId);
