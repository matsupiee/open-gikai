...
import type { MisatoShimaneMeeting } from "./list";
import {
  detectMeetingType,
  fetchBinary,
  toHalfWidth,
 ...
 *
 * 対応パターン:
 *   ○議長（田中一郎）　→ role=議長, name=田中一郎
 *   ○町長（山田太郎）　→ role=町長, name=山田太郎
 *   ○3番（鈴木花子）    → role=議員, name=鈴木花子
 *   ○教育長（中村次郎）→ role=教育長, name=中村次郎
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[324485-misato-shimane] PDF 取得失敗: ${pdfUrl}`,
