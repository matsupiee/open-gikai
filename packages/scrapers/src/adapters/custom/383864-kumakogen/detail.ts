...
export interface KumakogenDetailParams {
  title: string;
  heldOn: string | null;
  pdfUrl: string;
  meetingType: string;
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[383864-kumakogen] PDF 取得失敗: ${pdfUrl}`,
...
export async function buildMeetingData(
  ...
  return {
    ...
    externalId: `kumakogen_${params.meetingPageId}_${params.pdfUrl.split("/").pop()}`,
    statements,
  };
}
