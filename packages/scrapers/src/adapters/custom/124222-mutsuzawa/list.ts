 ...
 *
 * API エンドポイント:
 *   GET /dvsapi/councilrd/all?tenant_id=590&year={year}
 *
...
export interface MutsuzawaRecord {
  /** 会議 ID */
  councilId: string;
  /** 日程 ID */
  scheduleId: string;
 ...
 */
export function parseCouncilList(data: unknown): MutsuzawaRecord[] {
  if (!Array.isArray(data)) return [];

  const results: MutsuzawaRecord[] = [];
  ...
  for (const council of data as DvsCouncil[]) {
    ...
    const sourceUrl = `${BASE_ORIGIN}/smart/tenant/mutsuzawa/WebView/rd/council_1.html`;
 ...
 */
export async function fetchDocumentList(year: number): Promise<MutsuzawaRecord[]> {
  const url = `${BASE_ORIGIN}/dvsapi/councilrd/all?tenant_id=${TENANT_ID}&year=${year}`;
  const data = await fetchJson(url);
  if (!data) {
    console.warn(`[124222-mutsuzawa] No data returned for year=${year}`);
    return [];
  }
