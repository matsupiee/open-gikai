...
describe("parseSpeaker", () => {
  it("議長（名前君）パターンを解析する", () => {
    const result = parseSpeaker(
      "○議長（山口昌靖君）　ただいまから本日の会議を開きます。"
    );
    expect(result.speakerName).toBe("山口昌靖");
  ...
  it("町長（名前君）パターンを解析する", () => {
    ...
    expect(result.speakerName).toBe("西脇洋貴");
...
`;
    ...
    expect(statements[0]!.speakerName).toBe("山口昌靖");
    ...
    expect(statements[2]!.speakerName).toBe("西脇洋貴");
...
○３番（山田太郎君）　質問です。`;
  ...
  it("ト書き（登壇）はスキップする", () => {
    const text = `○議長（山口昌靖君）　ただいまから会議を開きます。
（３番　山田太郎君登壇）
○３番（山田太郎君）　質問があります。`;
