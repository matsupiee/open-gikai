...
export const BASE_ORIGIN = "https://www.town.miyagi-matsushima.lg.jp";

/** 定例会・臨時会（会議録一覧）ページのパス */
export const LIST_PATH = "/index.cfm/15,12059,27,html";
...
export async function fetchPage(url: string): Promise<string | null> {
  try {
    ...
    console.warn(`[044016-matsushima] fetchPage failed: ${url}`, e);
    return null;
  }
}
...
export async function fetchBinary(url: string): Promise<ArrayBuffer | null> {
  try {
    ...
    console.warn(`[044016-matsushima] fetchBinary failed: ${url}`, e);
    return null;
  }
}
