...
describe("parseHeldOn", () => {
  ...
  it("年なし月日 + fallbackYear でYYYY-MM-DDを生成する", () => {
    expect(parseHeldOn("3月4日", 2024)).toBe("2024-03-04");
    expect(parseHeldOn("12月10日", 2024)).toBe("2024-12-10");
    expect(parseHeldOn("9月10日", 2023)).toBe("2023-09-10");
  });

  it("fallbackYearがなければ月日のみではnullを返す", () => {
    expect(parseHeldOn("3月4日")).toBeNull();
    expect(parseHeldOn("12月10日")).toBeNull();
  });
});
...
describe("parseSpeaker", () => {
  ...
  it("役職が不明なテキストはspeakerRole=nullを返す", () => {
    const result = parseSpeaker("玉城町議会第１回定例会");
