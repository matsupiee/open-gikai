 ...
 * 玉城町議会 -- detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、発言者・発言内容をパースする。
...
import type { TamakiPdfRecord } from "./list";

export type TamakiDetailParams = {
  title: string;
  meetingType: string;
  pdfUrl: string;
  year: number;
...
export async function buildMeetingData(
  ...
  // externalId: tocPageUrl の末尾ファイル名 + pdfUrl のファイル名
  const tocPageFile = params.tocPageUrl.replace(/\/$/, "").split("/").pop() ?? "";
  const pdfFileName = new URL(params.pdfUrl).pathname.split("/").pop() ?? "";
  const externalId = `tamaki_${tocPageFile}_${pdfFileName}`;
...
/** TamakiPdfRecord を TamakiDetailParams に変換する型ヘルパー */
export function toDetailParams(record: TamakiPdfRecord): TamakiDetailParams {
  return {
    title: record.title,
