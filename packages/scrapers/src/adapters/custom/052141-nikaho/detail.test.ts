...
describe("parseSpeaker", () => {
  it("議長を正しくパースする", () => {
    const result = parseSpeaker("○議長（鈴木 誠） ただいまから本日の会議を開きます。");
    expect(result.speakerName).toBe("鈴木誠");
    expect(result.speakerRole).toBe("議長");
    expect(result.content).toBe("ただいまから本日の会議を開きます。");
  });

  it("市長を正しくパースする", () => {
    const result = parseSpeaker("○市長（市川雄次） お答えいたします。");
    expect(result.speakerName).toBe("市川雄次");
    expect(result.speakerRole).toBe("市長");
    expect(result.content).toBe("お答えいたします。");
  });

  it("番号付き議員を正しくパースする", () => {
    const result = parseSpeaker("○３番（田中一郎） 質問いたします。");
  ...
  it("総務部長を正しくパースする", () => {
    const result = parseSpeaker("○総務部長（佐々木健） ご説明いたします。");
    expect(result.speakerName).toBe("佐々木健");
    expect(result.speakerRole).toBe("部長");
    expect(result.content).toBe("ご説明いたします。");
  });

  it("総務常任委員長を正しくパースする", () => {
    const result = parseSpeaker("○総務常任委員長（山田花子） 報告をいたします。");
    expect(result.speakerName).toBe("山田花子");
    expect(result.speakerRole).toBe("委員長");
    expect(result.content).toBe("報告をいたします。");
  });

  it("副市長を正しくパースする", () => {
    const result = parseSpeaker("○副市長（佐藤次郎） ご報告申し上げます。");
  ...
  it("教育長を正しくパースする", () => {
    const result = parseSpeaker("○教育長（高橋正義） 行政報告を申し上げます。");
    expect(result.speakerName).toBe("高橋正義");
    expect(result.speakerRole).toBe("教育長");
    expect(result.content).toBe("行政報告を申し上げます。");
  });

  it("副委員長を正しくパースする", () => {
    const result = parseSpeaker("○産業建設常任副委員長（伊藤三郎） ご報告します。");
  ...
  it("全角数字の議員番号を正しくパースする", () => {
    const result = parseSpeaker("○１番（佐藤花子） 質問いたします。");
...
○３番（田中一郎） 質問いたします。
    ...
    expect(statements[0]!.speakerName).toBe("鈴木誠");
    expect(statements[0]!.speakerRole).toBe("議長");
    expect(statements[0]!.kind).toBe("remark");
    expect(statements[0]!.content).toBe("ただいまから本日の会議を開きます。");

    expect(statements[1]!.speakerName).toBe("市川雄次");
...
○議長（鈴木 誠） 本日の会議を開きます。
  ...
  it("contentHash が SHA-256 で生成される", () => {
    const text = "○議長（鈴木 誠） ただいまから会議を開きます。";
  ...
  it("statements が空の場合は空配列を返す", () => {
    const text = "令和６年第３回にかほ市議会定例会会議録";
