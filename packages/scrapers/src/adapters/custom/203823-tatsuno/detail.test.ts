...
describe("parseHeldOn", () => {
  it("令和の日付を抽出する", () => {
    const text = "令和6年12月10日\n辰野町議会定例会\n○議長（田中君） 開会します。";
    expect(parseHeldOn(text, 2024)).toBe("2024-12-10");
  });

  it("平成の日付を抽出する", () => {
    const text = "平成17年3月1日\n辰野町議会定例会\n○議長（鈴木君） 開会します。";
    expect(parseHeldOn(text, 2005)).toBe("2005-03-01");
  });
  ...
  it("日付が見つからない場合はnullを返す", () => {
    expect(parseHeldOn("辰野町議会会議録", 2024)).toBeNull();
