...
describe("parseSpeaker", () => {
  ...
  it("町長（名前君）パターンを解析する", () => {
    const result = parseSpeaker("○町長（山本花子君）　お答えいたします。");
  ...
  it("副町長パターンを解析する", () => {
    const result = parseSpeaker("○副町長（中村五郎君）　ご報告いたします。");
  ...
  it("名前に空白を含む場合は除去される", () => {
    const result = parseSpeaker("○町長（山本　花子君）　答弁します。");
    expect(result.speakerName).toBe("山本花子");
  });
...
describe("extractHeldOn", () => {
  it("令和の日付を抽出する", () => {
    const text =
      "令和7年2月24日\n高取町議会第4回定例会会議録\n○議長（田中一郎君）　開会します。";
    expect(extractHeldOn(text)).toBe("2025-02-24");
  });

  it("令和6年の日付を抽出する", () => {
    const text =
      "令和6年8月23日\n高取町議会第2回定例会会議録\n○議長（田中一郎君）　開会します。";
    expect(extractHeldOn(text)).toBe("2024-08-23");
  });

  it("全角数字を含む日付を抽出する", () => {
    const text = "令和６年１２月１６日 高取町議会第３回定例会会議録";
    expect(extractHeldOn(text)).toBe("2024-12-16");
  });
