...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[294012-takatori] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(params: {
  ...
  // 開催日: PDF ラベル（例: "本会議　2月24日"）または PDF 本文から抽出
  const heldOn = extractHeldOn(text);
  if (!heldOn) {
    console.warn(`[294012-takatori] 開催日の抽出失敗: ${pdf.pdfUrl}`);
    return null;
  }

  // タイトル: 会議名 + PDF ラベル
  const title = pdf.label
    ? `${session.title} ${pdf.label}`.trim()
    : session.title;

  // externalId: frmId + ファイル名
  const fileMatch = pdf.pdfUrl.match(/\/([^/]+)\.pdf$/i);
