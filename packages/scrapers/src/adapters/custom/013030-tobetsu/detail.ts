...
import type { TobetsuMeeting } from "./list";
import {
  detectMeetingType,
  fetchBinary,
  toHalfWidth,
 ...
 *   ○町長（田中一郎）　→ role=町長, name=田中一郎
 *   ○5番（佐藤次郎）　→ role=議員, name=佐藤次郎
 *   ○教育長（鈴木三郎）→ role=教育長, name=鈴木三郎
 */
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[013030-tobetsu] PDF 取得失敗: ${pdfUrl}`,
