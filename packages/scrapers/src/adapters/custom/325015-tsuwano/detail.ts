...
import type { TsuwanoMeeting } from "./list";
import {
  LIST_URL,
  buildExternalId,
  detectMeetingType,
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[325015-tsuwano] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = buildExternalId(meeting.filename);
