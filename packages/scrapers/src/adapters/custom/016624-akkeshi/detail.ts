/**
 * 厚岸町議会 会議録 — detail フェーズ
 *
 * PDF をダウンロードしてテキストを抽出し、○ マーカーで発言を分割して
 * ParsedStatement 配列を生成する。
 *
 * 発言フォーマット（推定 — PDF テキスト抽出結果に基づく）:
...
import type { AkkeshiMeeting } from "./list";
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[016624-akkeshi] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  const externalId = idKey ? `akkeshi_${idKey}` : null;
