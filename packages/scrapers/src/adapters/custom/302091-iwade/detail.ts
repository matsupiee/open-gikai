/**
 * 岩出市議会 会議録 — detail フェーズ
 *
 * 全体版 PDF をダウンロードし、テキストを抽出して発言を構造化データに変換する。
 *
 * PDF のテキストは全文字間にスペースが挟まれているため、除去して処理する。
 * 発言は「○発言者名役職 発言内容」の形式で始まる。
 */
...
/**
 * 全角数字を半角数字に変換する。
 * PDF から抽出したテキストには全角数字が含まれる場合がある。
 */
 ...
 *
 * フォーマット（スペース除去後）: "○玉田議長ただいまから本日の会議を開きます。"
 *
 * 役職が名前の末尾に付く形式: 「玉田議長ただいまから...」→ name=玉田, role=議長, content=ただいまから...
 *
 * 戦略: 役職サフィックス（長い順）を先頭から検索し、最初にマッチしたもので分割する。
 */
export function parseSpeaker(text: string): {
  ...
  for (const suffix of ROLE_SUFFIXES) {
    ...
    // 名前部分が長すぎる場合はスキップ（役職名が固有名詞の一部の可能性）
    if (idx > MAX_NAME_LEN) continue;

    const nameEnd = idx + suffix.length;
    // 役職名の直後が「会」「団」「長」など組織名の一部になりえる文字の場合はスキップ
    // 例: 「議長会」「委員会」「事務局長会」
  ...
  // 発言者名は通常漢字で構成されるため、漢字以外（ひらがな・カタカナ・句読点等）が
  // 現れた時点を名前の終わりとする
  if (/^[○◯◎●]/.test(text)) {
...
export async function fetchPdfStatements(
  ...
  try {
    ...
    console.warn("[302091-iwade] PDF text extraction failed:", pdfUrl, e);
    return null;
  }
...
export async function fetchMeetingData(
  ...
  const { statements, heldOn } = result;
