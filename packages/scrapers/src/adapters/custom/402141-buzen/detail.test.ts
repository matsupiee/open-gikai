...
describe("parseSpeaker", () => {
  ...
  it("市長（名前君）パターンを解析する", () => {
    const result = parseSpeaker("○市長（山田花子君）　お答えいたします。");
  ...
  it("番号議員パターンを解析する", () => {
    const result = parseSpeaker("○１番（佐藤一郎君）　質問いたします。");
  ...
  it("教育長パターンを解析する", () => {
    const result = parseSpeaker("○教育長（鈴木次郎君）　お答えいたします。");
  ...
  it("名前に空白を含む場合は除去される", () => {
    const result = parseSpeaker("○市長（山田　花子君）　答弁します。");
    expect(result.speakerName).toBe("山田花子");
  });
