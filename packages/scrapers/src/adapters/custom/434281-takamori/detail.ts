...
import type { TakamorivMeeting } from "./list";
import {
  fetchBinary,
  detectMeetingType,
  parseJapaneseDate,
...
async function fetchPdfText(pdfUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `[434281-takamori] PDF 取得失敗: ${pdfUrl}`,
...
export async function fetchMeetingData(
  ...
  // PDF テキストから開催日を取得を試みる
  const heldOnFromPdf = parseJapaneseDate(text);
  const heldOn = heldOnFromPdf ?? meeting.heldOn;

  // heldOn が解析できない場合は null を返す（"1970-01-01" 禁止）
  if (!heldOn) return null;

  // 年度別ページ URL を sourceUrl とする
  const sourceUrl = meeting.yearPageUrl;

  const externalId = `takamori_${meeting.attachmentId}`;
