...
- `--target {code1},{code2}` を指定すると対象自治体を絞り込める（初回は全件実行が基本）
...
- まだ 0 件の自治体があれば Step 3 に戻って原因を再調査し、修正をやり直す

#### Step 5b: デグレチェック — 修正していない自治体もサンプル確認する (CRITICAL)

修正が他の自治体に影響していないか確認するため、**同じ system_type の未修正自治体から 3〜5 件をランダムに選んで `--target` で実行する**:
...
- **同じ system_type** かつ Step 1 で正常に取得できていた自治体を選ぶこと（異なる system_type ではデグレチェックにならない）
- もしデグレが見つかった場合は修正をロールバックし、影響範囲を再調査する

#### Step 5c: まだ未修正の自治体があれば Step 1 に戻る

5 件修正・確認した時点で修正作業を止め、**Step 1 に戻って全件再実行する**。0 件の自治体がなくなるか、残りがすべてカテゴリ C（対処不要）になるまでこのループを繰り返す。
